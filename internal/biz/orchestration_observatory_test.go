...
func TestBuildTeamRunObservatory_FromSteps(t *testing.T) {
	...
	run := TeamRun{ID: "run-1", TeamID: "t1", SessionID: "s1", Status: "success", Mode: "sequential"}
	steps := []TeamRunStep{
...
func TestHasActiveTeamRun(t *testing.T) {
	if !HasActiveTeamRun([]TeamRun{{Status: "running"}}) {
		t.Fatal("expected active")
	}
	if HasActiveTeamRun([]TeamRun{{Status: "success"}}) {
