...
// seedBucket overwrites one bucket's counters (used by Engine.RebuildRingBuffer
// when replaying persisted counts into the ring).
func (rb *MetricRingBuffer) seedBucket(startUnix int64, total, errors int64) {
	rb.mu.Lock()
	b := rb.ensureBucketAt(startUnix)
	b.totals["runner.completion"] = total
	b.errors["runner.completion"] = errors
	rb.mu.Unlock()
}

// BucketSize exposes the ring's bucket granularity for rebuild window math.
func (rb *MetricRingBuffer) BucketSize() time.Duration {
	if rb == nil {
		return defaultBucketSize
	}
	return rb.bucketSize
}
