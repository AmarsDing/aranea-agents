...
// EventCounter is the narrow read port the alert domain needs from the
// monitor event store (satisfied structurally by monitor.EventRepo).
...
func NewRunnerErrorRateMetric(repo EventCounter, rb *MetricRingBuffer) *RunnerErrorRateMetric {
...
// defined here so the alert domain does not depend on the agent package
