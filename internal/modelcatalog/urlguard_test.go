...
func TestValidateCatalogSourceURL(t *testing.T) {
	if err := ValidateCatalogSourceURL("https://models.dev/api.json"); err != nil {
		t.Fatalf("models.dev should be allowed: %v", err)
	}
	if err := ValidateCatalogSourceURL("https://models.dev/models.json"); err == nil {
		t.Fatal("models.json should be rejected")
	}
	if err := ValidateCatalogSourceURL("http://models.dev/api.json"); err == nil {
		t.Fatal("http should be rejected")
	}
	if err := ValidateCatalogSourceURL("https://127.0.0.1/x"); err == nil {
...
func TestShouldSkipCatalogApply(t *testing.T) {
	if !shouldSkipCatalogApply(map[string]any{"catalog_source": "custom"}, "") {
		t.Fatal("custom source should skip")
	}
	if !shouldSkipCatalogApply(map[string]any{"catalog_managed": false}, "") {
		t.Fatal("catalog_managed=false should skip")
	}
	if !shouldSkipCatalogApply(map[string]any{}, `{"catalog_source":"custom"}`) {
		t.Fatal("metadata custom should skip")
	}
	if shouldSkipCatalogApply(map[string]any{"catalog_managed": true, "catalog_source": "models.dev"}, "") {
