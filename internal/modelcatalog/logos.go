...
// LogoSyncResult summarizes provider logo cache refresh.
...
// SyncProviderLogos downloads provider SVG logos into {store}/logos/{provider_id}.svg.
func SyncProviderLogos(ctx context.Context, store *Store, cat Catalog, logosBaseURL string) LogoSyncResult {
...
// ProviderLogoURL returns the local API path for a cached provider logo.
