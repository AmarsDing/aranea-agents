...
func TestAssemblyForCatalogKey(t *testing.T) {
	...
	cfg, ok := AssemblyForCatalogKey("read_file", nil, nil, noop)
	...
	cfg, ok = AssemblyForCatalogKey("patch_file", nil, nil, noop)
	if !ok || cfg.EnabledTools[0] != "file" {
		t.Fatalf("patch_file cfg=%+v ok=%v", cfg, ok)
	}
	_, ok = AssemblyForCatalogKey("mcp_tool_set", nil, nil, noop)
	if ok {
...
func TestAssemblyForCatalogKey_webResearchPlatform(t *testing.T) {
	noop := loggateway.NewNoop()
	_, ok := AssemblyForCatalogKey("web_research", map[string]any{"provider": "tavily"}, nil, noop)
	if ok {
