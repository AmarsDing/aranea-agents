...
var stepTitleRegistry = map[string]string{
	...
	"chat.turn.awaiting_input":             "对话轮次挂起等待澄清",
	"chat.orch.decision":                   "编排路由决策",
	...
	"system.memory.aux_extract":            "aux_memory_extract 完成",
	"system.gate":                          "闸/HITL 决策",
	"system.gate.unknown":                  "闸决策（未标注触发词）",
	"system.gate.collector_nil":            "闸决策 collector 未接线",
	"system.gate.hitl_approval":            "HITL 审批决策",
	"system.gate.input_risk_flagged":       "输入安全扫描命中",
	"system.gate.loop_guard_blocked":       "循环守卫拦截",
	"system.gate.no_progress_tripped":      "无进展熔断",
	"system.gate.token_budget_tripped":     "Token 预算闸",
	"system.gate.param_rule_deny":          "参数规则拦截",
	"system.memory_canary.failed":          "记忆闭环金丝雀告警",
	"system.memory_l1_archive.failed":      "L1 归档连续失败告警",
	"system.channel_delivery.failed":       "渠道投递失败",
	"system.channel_health.failed":         "渠道健康检查失败",
	"system.provider_health.failed":        "模型供应商健康检查失败",
	"system.auto_heal_ttl_cleanup.failed":  "自愈记录清理失败",
	...
	"team.session.resumed":                 "唤醒挂起会话",
	"team.member":                          "团队成员执行",
	"team.model_cascade.route":             "成员模型级联路由",
	"team.eval_profile.applied":            "评测态 profile 生效",
	"team.quality_gate.revise":             "交付物质量门打回修订",
	"team.quality_gate.bypass":             "交付物质量门降级放行",
	...
	"graph.replan.decided":                 "图重规划决策",
	"graph.replan.applied":                 "图重规划落地",
	...
	"knowledge.collection.enable_semantic": "集合语义层启用",
	...
	"agent.crud.batch_update":    "Agent 批量更新",
	"provider.catalog.sync":      "模型目录同步",
	"mcp.server.add":             "MCP 服务器添加",
	"mcp.server.update":          "MCP 服务器更新",
