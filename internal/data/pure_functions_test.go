...
func TestSnapshotFromUSD_Zero(t *testing.T) {
	snap := snapshotFromUSD(modelregistry.CostUSDPer1M{})
	if snap.InputPriceUSDPer1M != 0 || snap.OutputPriceUSDPer1M != 0 {
		t.Fatalf("expected zero USD fields, got input=%v output=%v", snap.InputPriceUSDPer1M, snap.OutputPriceUSDPer1M)
	}
	if snap.InputPriceMicroUSDPer1K != 0 || snap.OutputPriceMicroUSDPer1K != 0 {
		t.Fatalf("expected zero micro fields, got input=%d output=%d", snap.InputPriceMicroUSDPer1K, snap.OutputPriceMicroUSDPer1K)
	}
}

func TestSnapshotFromUSD_NonZero(t *testing.T) {
	cost := modelregistry.CostUSDPer1M{Input: 30, Output: 60}
	snap := snapshotFromUSD(cost)
	if snap.InputPriceUSDPer1M != 30 {
		t.Fatalf("expected input USD 30, got %v", snap.InputPriceUSDPer1M)
	}
	if snap.OutputPriceUSDPer1M != 60 {
		t.Fatalf("expected output USD 60, got %v", snap.OutputPriceUSDPer1M)
	}
	if snap.InputPriceMicroUSDPer1K <= 0 {
		t.Fatalf("expected positive input micro, got %d", snap.InputPriceMicroUSDPer1K)
	}
	if snap.OutputPriceMicroUSDPer1K <= 0 {
		t.Fatalf("expected positive output micro, got %d", snap.OutputPriceMicroUSDPer1K)
	}
}

func TestSnapshotFromProviderConfig_Empty(t *testing.T) {
	snap := snapshotFromProviderConfig(providerModelPricingJSON{})
	if snap.InputPriceUSDPer1M != 0 || snap.OutputPriceUSDPer1M != 0 {
		t.Fatalf("expected zero snapshot for empty config, got %+v", snap)
	}
}

func TestSnapshotFromProviderConfig_LegacyMicro(t *testing.T) {
	cfg := providerModelPricingJSON{
		InputPriceMicroUSDPer1K:  30000,
		OutputPriceMicroUSDPer1K: 60000,
	}
	snap := snapshotFromProviderConfig(cfg)
	if snap.InputPriceUSDPer1M <= 0 {
		t.Fatalf("expected positive input USD, got %v", snap.InputPriceUSDPer1M)
	}
	if snap.OutputPriceUSDPer1M <= 0 {
		t.Fatalf("expected positive output USD, got %v", snap.OutputPriceUSDPer1M)
	}
}

func TestSnapshotFromProviderConfig_FullUSD(t *testing.T) {
	cfg := providerModelPricingJSON{
		Cost: providerCostJSON{
			InputUSDPer1M:  30,
			OutputUSDPer1M: 60,
		},
	}
