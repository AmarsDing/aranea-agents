...
type CronTask struct {
	...
	// TaskKey holds the value of the "task_key" field.
...
func (_m *CronTask) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case crontask.FieldAgentID:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field agent_id", values[i])
			} else if value.Valid {
				_m.AgentID = value.String
			}
		case crontask.FieldConfigJSON:
			if value, ok := values[i].(*sql.NullString); !ok {
...
func (_m *CronTask) Value(name string) (ent.Value, error) {
...
func (_m *CronTask) Update() *CronTaskUpdateOne {
	return NewCronTaskClient(_m.config).UpdateOne(_m)
}

// Unwrap unwraps the CronTask entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (_m *CronTask) Unwrap() *CronTask {
	_tx, ok := _m.config.driver.(*txDriver)
	if !ok {
		panic("ent: CronTask is not a transactional entity")
	}
...
func (_m *CronTask) String() string {
