...
var builtinPlatformToolSeeds = []platformToolSeed{
	...
	{key: "computer_use_act", displayName: "桌面动作", description: "在桌面执行语义动作：给出目标元素的自然语言描述，系统自动定位并操作（invoke/click/type/key）。支持 dry_run 干跑；高危动作强制人工确认。", category: "computeruse", riskLevel: "high", enabled: false, reqConfirm: true, paramsSchema: `{"type":"object","properties":{"target":{"type":"string"},"action":{"type":"string","enum":["invoke","click","type","key"]},"text":{"type":"string"},"combo":{"type":"string"},"x":{"type":"integer"},"y":{"type":"integer"},"button":{"type":"string","enum":["left","right","middle"]},"click_count":{"type":"integer"},"dry_run":{"type":"boolean"},"session_id":{"type":"string"},"confirmed_by":{"type":"string"}}}`},
