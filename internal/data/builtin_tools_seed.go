...
var builtinPlatformToolSeeds = []platformToolSeed{
	...
	// 会话沙箱文件工具族（M82 P1-2）：与本会话 execute_code 共享同一用完即毁沙箱
	// （internal/tools/sandboxfs，经共享 sandbox.SessionLeases 装配为 customTools，
	// 无 registryName）。沙箱子系统禁用或 docker daemon 不可用时装配层整体裁剪，
	// agent 看不到必然失败的工具。种子 enabled=false opt-in：按 Agent allow JSON /
	// profile 显式授予（group:sandbox），默认不污染通用工具面。
	{key: "sandbox_fs_write", displayName: "沙箱写文件", description: "在会话沙箱中写文件（自动创建父目录）。沙箱与本会话的 execute_code 共享：写入的文件可被后续代码执行读取；/workspace/out/ 下的文件会作为代码执行产物被收集。路径仅允许 /tmp/ 与 /workspace/out/ 前缀。沙箱用完即毁，会话结束后文件不保留。", category: "sandbox", riskLevel: "medium", enabled: false, paramsSchema: `{"type":"object","properties":{"path":{"type":"string","description":"沙箱内绝对路径；仅可写 /tmp/ 与 /workspace/out/ 前缀（rootfs 只读）"},"content":{"type":"string","description":"文件内容"},"encoding":{"type":"string","enum":["utf8","base64"],"description":"content 编码，默认 utf8；二进制内容用 base64"}},"required":["path","content"]}`},
	{key: "sandbox_fs_read", displayName: "沙箱读文件", description: "读取会话沙箱中的文件内容（与本会话的 execute_code 同一沙箱）。文本按 UTF-8 返回，二进制自动 base64（encoding 字段标识）；超长按 max_bytes 截断（truncated=true）。", category: "sandbox", riskLevel: "low", enabled: false, readonly: true, paramsSchema: `{"type":"object","properties":{"path":{"type":"string","description":"沙箱内绝对路径"},"max_bytes":{"type":"integer","description":"最大返回字节数，默认 65536，上限 262144；超出截断"}},"required":["path"]}`},
