...
var builtinPlatformToolSeeds = []platformToolSeed{
	...
	{key: "working_memory_patch", displayName: "工作记忆批量补丁", description: "一次写入多个字段。任意一项失败将中断后续写入并返回错误。", category: "memory", enabled: true, paramsSchema: `{"type":"object","properties":{"patches":{"type":"array","items":{"type":"object","properties":{"field_path":{"type":"string"},"value":{},"field_kind":{"type":"string"},"visibility":{"type":"string"},"reason":{"type":"string"},"if_revision":{"type":"integer"}},"required":["field_path","value"]}}},"required":["patches"]}`},
	...
	{key: "assemble_team", displayName: "组建团队", description: "[DEPRECATED] 当用户需求复杂、需要多 Agent 协作时，组建任务团队。请改用 plan_and_execute。", category: "spirit", source: "builtin", riskLevel: "low", enabled: true, readonly: true, paramsSchema: `{"type":"object","properties":{"agent_keys":{"type":"array","items":{"type":"string"},"description":"参与团队的 Agent key 列表"},"mode":{"type":"string","enum":["coordinator","sequential","parallel"],"description":"团队编排模式"},"task_prompt":{"type":"string","description":"任务描述"}},"required":["task_prompt"]}`},
	{key: "assess_complexity", displayName: "评估任务复杂度", description: "[DEPRECATED] 评估用户消息的任务复杂度。请改用 plan_and_execute。", category: "spirit", source: "builtin", riskLevel: "low", enabled: true, readonly: true, paramsSchema: `{"type":"object","properties":{"user_message":{"type":"string","description":"用户消息内容"}},"required":["user_message"]}`},
	{key: "list_butlers", displayName: "列出管家", description: "[DEPRECATED] 列出可用的管家列表。请改用 plan_and_execute。", category: "spirit", source: "builtin", riskLevel: "low", enabled: true, readonly: true, paramsSchema: `{"type":"object","properties":{},"required":[]}`},
	{key: "query_butler_status", displayName: "查询管家状态", description: "[DEPRECATED] 查询指定管家的当前状态。请改用 plan_and_execute。", category: "spirit", source: "builtin", riskLevel: "low", enabled: true, readonly: true, paramsSchema: `{"type":"object","properties":{"butler_name":{"type":"string","description":"管家名称"}},"required":["butler_name"]}`},
	{key: "check_team_progress", displayName: "查询团队进度", description: "[DEPRECATED] 查询当前精灵会话下所有团队的执行进度。请改用 check_progress。", category: "spirit", source: "builtin", riskLevel: "low", enabled: true, readonly: true, paramsSchema: `{"type":"object","properties":{},"required":[]}`},
	{key: "cancel_team", displayName: "取消团队", description: "[DEPRECATED] 取消正在运行的团队。请改用 cancel_orchestration。", category: "spirit", source: "builtin", riskLevel: "medium", enabled: true, readonly: true, paramsSchema: `{"type":"object","properties":{"team_id":{"type":"string","description":"要取消的团队 ID"}},"required":["team_id"]}`},
	{key: "plan_and_execute", displayName: "规划并执行", description: "规划并执行任务。自动评估复杂度、分配 Agent、启动编排。替代 assess_complexity + assemble_team 的组合调用。", category: "spirit", source: "builtin", riskLevel: "low", enabled: true, readonly: true, paramsSchema: `{"type":"object","properties":{"task_prompt":{"type":"string","description":"The task to plan and execute"},"mode":{"type":"string","description":"Execution mode: auto|direct|single|parallel|dag|coordinator (default: auto)"}},"required":["task_prompt"]}`},
	{key: "check_progress", displayName: "查询编排进度", description: "查询编排执行进度。基于 orchestration_id 查询，替代 check_team_progress。", category: "spirit", source: "builtin", riskLevel: "low", enabled: true, readonly: true, paramsSchema: `{"type":"object","properties":{"orchestration_id":{"type":"string","description":"The orchestration ID to check progress for"}},"required":["orchestration_id"]}`},
	{key: "cancel_orchestration", displayName: "取消编排", description: "取消正在运行的编排。基于 orchestration_id 取消，替代 cancel_team。", category: "spirit", source: "builtin", riskLevel: "medium", enabled: true, readonly: true, paramsSchema: `{"type":"object","properties":{"orchestration_id":{"type":"string","description":"The orchestration ID to cancel"}},"required":["orchestration_id"]}`},
