...
var builtinPlatformToolSeeds = []platformToolSeed{
	...
	{key: "search_content", displayName: "内容搜索", description: "在工作区内搜索文本内容（优先 ripgrep，结果带路径与行号；过长结果会截断）。", category: "filesystem", enabled: true, readonly: true, paramsSchema: `{"type":"object","properties":{"content_pattern":{"type":"string","description":"正则表达式搜索模式"},"file_pattern":{"type":"string","description":"文件 glob 匹配模式，如 *.go"},"path":{"type":"string","description":"搜索根目录"},"file_case_sensitive":{"type":"boolean","description":"文件匹配是否区分大小写"},"content_case_sensitive":{"type":"boolean","description":"内容匹配是否区分大小写"}},"required":["content_pattern","file_pattern"]}`, registryName: "file"},
	{key: "read_lints", displayName: "读取诊断", description: "读取已改文件的编译/静态检查诊断（Go 走 go vet）。改完代码后调用，不要猜测是否能编译。", category: "filesystem", enabled: true, readonly: true, paramsSchema: `{"type":"object","properties":{"path":{"type":"string","description":"工作区相对路径（文件或目录）"},"paths":{"type":"array","items":{"type":"string"},"description":"可选，多个路径"}}}`, registryName: "read_lints"},
