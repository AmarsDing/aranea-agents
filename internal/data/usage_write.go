...
func upsertModelTokenUsageDaily(ctx context.Context, c execer, dialect Dialect, e biz.TokenUsageEvent) error {
	...
	_, err := c.ExecContext(ctx,
	 ...
	 call_count = model_token_usage_daily.call_count + excluded.call_count,
	 request_count = model_token_usage_daily.request_count + excluded.request_count,
	 success_count = model_token_usage_daily.success_count + excluded.success_count,
	 failed_count = model_token_usage_daily.failed_count + excluded.failed_count,
	 cancelled_count = model_token_usage_daily.cancelled_count + excluded.cancelled_count,
	 input_tokens = model_token_usage_daily.input_tokens + excluded.input_tokens,
	 output_tokens = model_token_usage_daily.output_tokens + excluded.output_tokens,
	 cached_input_tokens = model_token_usage_daily.cached_input_tokens + excluded.cached_input_tokens,
	 reasoning_tokens = model_token_usage_daily.reasoning_tokens + excluded.reasoning_tokens,
	 embedding_tokens = model_token_usage_daily.embedding_tokens + excluded.embedding_tokens,
	 total_tokens = model_token_usage_daily.total_tokens + excluded.total_tokens,
	 total_cost_micro_usd = model_token_usage_daily.total_cost_micro_usd + excluded.total_cost_micro_usd,
	 avg_latency_ms = (model_token_usage_daily.avg_latency_ms * model_token_usage_daily.request_count + excluded.avg_latency_ms * excluded.request_count) / (model_token_usage_daily.request_count + excluded.request_count),
	 avg_tokens_per_second = (model_token_usage_daily.avg_tokens_per_second * model_token_usage_daily.request_count + excluded.avg_tokens_per_second * excluded.request_count) / (model_token_usage_daily.request_count + excluded.request_count),
...
func upsertModelTokenUsageHourly(ctx context.Context, c execer, dialect Dialect, e biz.TokenUsageEvent) error {
	...
	_, err := c.ExecContext(ctx,
	 ...
	 call_count = model_token_usage_hourly.call_count + excluded.call_count,
	 request_count = model_token_usage_hourly.request_count + excluded.request_count,
	 success_count = model_token_usage_hourly.success_count + excluded.success_count,
	 failed_count = model_token_usage_hourly.failed_count + excluded.failed_count,
	 cancelled_count = model_token_usage_hourly.cancelled_count + excluded.cancelled_count,
	 input_tokens = model_token_usage_hourly.input_tokens + excluded.input_tokens,
	 output_tokens = model_token_usage_hourly.output_tokens + excluded.output_tokens,
	 cached_input_tokens = model_token_usage_hourly.cached_input_tokens + excluded.cached_input_tokens,
	 reasoning_tokens = model_token_usage_hourly.reasoning_tokens + excluded.reasoning_tokens,
	 embedding_tokens = model_token_usage_hourly.embedding_tokens + excluded.embedding_tokens,
	 total_tokens = model_token_usage_hourly.total_tokens + excluded.total_tokens,
	 total_cost_micro_usd = model_token_usage_hourly.total_cost_micro_usd + excluded.total_cost_micro_usd,
	 avg_latency_ms = (model_token_usage_hourly.avg_latency_ms * model_token_usage_hourly.request_count + excluded.avg_latency_ms * excluded.request_count) / (model_token_usage_hourly.request_count + excluded.request_count),
	 avg_tokens_per_second = (model_token_usage_hourly.avg_tokens_per_second * model_token_usage_hourly.request_count + excluded.avg_tokens_per_second * excluded.request_count) / (model_token_usage_hourly.request_count + excluded.request_count),
	 updated_at = excluded.updated_at`),
