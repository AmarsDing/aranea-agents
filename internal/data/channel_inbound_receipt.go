...
func PurgeOldInboundReceipts(ctx context.Context, db *sql.DB, dialect Dialect, retention time.Duration) error {
	if db == nil || retention <= 0 {
		return nil
	}
	cutoff := time.Now().UTC().Add(-retention).Format(time.RFC3339)
	_, err := db.ExecContext(ctx, dialect.RenumberPlaceholders(`DELETE FROM channel_inbound_receipt WHERE created_at < ? AND created_at != ''`), cutoff)
	return err
}
