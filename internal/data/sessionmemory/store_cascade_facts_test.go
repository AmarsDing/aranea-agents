...
func openCascadeFactsStore(t *testing.T) *sessionmemory.Store {
	t.Helper()
	...
	stmt := `CREATE TABLE IF NOT EXISTS memory_facts (
 ...
 UNIQUE(scope_type, scope_id, fingerprint))`
	...
	_, _ = client.ExecContext(ctx, `CREATE TABLE IF NOT EXISTS memory_action_log (
...
func TestReplaceNameInAgentFacts_WholeWordOnly(t *testing.T) {
	store := openCascadeFactsStore(t)
	ctx := context.Background()
	raw, err := store.UpsertFactRow(ctx, sessionmemory.MemoryFactUpsert{
		ScopeType: "agent", ScopeID: "ag1", AgentID: "ag1",
		Statement: "Bob works with Bobson team", DetailsMarkdown: "Bob works with Bobson team",
		FactKind: "fact", Status: "active", SourceKind: "test",
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	updated, n, err := store.ReplaceNameInAgentFacts(ctx, "ag1", "Bob", "Alice")
	if err != nil {
		t.Fatal(err)
	}
	if n != 1 {
	...
	if err := json.Unmarshal(updated[0], &row); err != nil {
		t.Fatal(err)
	}
	stmt, _ := row["statement"].(string)
	if strings.Contains(stmt, "Bobson") == false {
		t.Fatalf("expected Bobson preserved, got %q", stmt)
	}
	if !strings.Contains(stmt, "Alice") || strings.Contains(stmt, " Bob ") {
		t.Fatalf("expected Bob->Alice whole word only, got %q", stmt)
	}
}
