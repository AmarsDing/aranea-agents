...
// ApplyAgentFactImportanceDecay batch-decays fact importance for one agent's stale L3 rows.
func (st *Store) ApplyAgentFactImportanceDecay(ctx context.Context, agentID, cutoffRFC3339 string, factor float64) (int, error) {
	if st == nil || st.client == nil {
		return 0, nil
	}
...
UPDATE memory_facts SET
 ...
 decay_factor = decay_factor * ?,
 updated_at = ?
WHERE agent_id = ? AND deleted_at = '' AND status = 'active' AND updated_at != '' AND updated_at < ?`,
		factor, factor, time.Now().UTC().Format(time.RFC3339Nano), agentID, cutoffRFC3339)
	if err != nil {
		return 0, err
	}
	n, _ := res.RowsAffected()
