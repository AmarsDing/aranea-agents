...
func TestGoroutinePool_Go(t *testing.T) {
	p := NewGoroutinePool()
	...
	p.Go(ctx, "test", func(ctx context.Context) {
		defer wg.Done()
	...
	if atomic.LoadInt32(&executed) != 1 {
		t.Fatal("goroutine not executed")
	}
}

func TestGoroutinePool_GoBackground(t *testing.T) {
	p := NewGoroutinePool()
	...
	p.GoBackground("test-bg", func(ctx context.Context) {
		defer wg.Done()
		atomic.StoreInt32(&executed, 1)
		// 验证 ctx 非 nil
		if ctx == nil {
			t.Error("background ctx should not be nil")
		}
	})

	wg.Wait()
	if atomic.LoadInt32(&executed) != 1 {
		t.Fatal("background goroutine not executed")
	}
}
...
func TestGoroutinePool_PanicRecovery(t *testing.T) {
	...
	p.SetPanicHook(func(name string, r interface{}, stack []byte) {
		atomic.StoreInt32(&hookCalled, 1)
		if name != "panic-test" {
			t.Errorf("hook name = %q, want %q", name, "panic-test")
		}
	})

	var wg sync.WaitGroup
	wg.Add(1)
	p.Go(context.Background(), "panic-test", func(ctx context.Context) {
		defer wg.Done()
		panic("test panic")
	})
...
func TestManagedCache_SetAndGet(t *testing.T) {
	c := NewManagedCache[string, int](0, 0)
	defer c.Close()
...
func TestManagedCache_TTL(t *testing.T) {
	c := NewManagedCache[string, int](100*time.Millisecond, 0)
	defer c.Close()

	c.Set("temp", 1)
	v, ok := c.Get("temp")
	if !ok || v != 1 {
		t.Fatalf("immediate Get = (%v, %v), want (1, true)", v, ok)
	}

	time.Sleep(150 * time.Millisecond)
	_, ok = c.Get("temp")
	if ok {
		t.Fatal("Get after TTL should return false")
	}
}

func TestManagedCache_MaxSize(t *testing.T) {
	c := NewManagedCache[string, int](0, 2)
	defer c.Close()
	...
	if total > 2 {
		t.Fatalf("total entries = %d, should be <= 2", total)
	}
}
...
func TestManagedCache_Close(t *testing.T) {
	...
	// Close 后仍可读写（只是停止后台清理）
	c.Set("b", 2)
	v, ok := c.Get("b")
	if !ok || v != 2 {
		t.Fatalf("Get after Close = (%v, %v), want (2, true)", v, ok)
	}
}
