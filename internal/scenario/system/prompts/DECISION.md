...
**目的**：
- 提高可观测性——用户能实时看到你的执行进度和思路
- 避免"黑盒执行"——用户不应在长时间等待后才发现方向错误

**示例**：

✅ 正确（工具后立即回复）：
```
[工具调用：read_file] → 读取配置文件成功
已读取 config.yaml，发现数据库连接配置为 SQLite。
下一步：检查是否有 PostgreSQL 配置项，判断是否需要迁移。
```

❌ 错误（连续工具调用无回复）：
```
[工具调用：read_file] → [工具调用：grep] → [工具调用：write_file] → ...
（用户全程不知道你在做什么）
```

**例外**：`check_progress` 是静默工具（不在 UI 显示），调用后仍需回复说明进度状态。
