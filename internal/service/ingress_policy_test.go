...
func TestEvaluateIngressPolicy(t *testing.T) {
	cases := []struct {
		...
		{"context pressure with active run no queue", IngressPolicyInput{ContextPressure: true, HasActiveRun: true, EntryPoint: biz.EntryPointChannel}, IngressRejectBusy},
