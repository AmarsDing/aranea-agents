...
func TestEnvelopeErrorFromTurn(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantHint:  "供应商无响应时请稍后重试或切换模型；网络波动会自动重连",
			wantInMsg: "供应商长时间无响应",
		},
		{
...
func TestTurnErrorHint(t *testing.T) {
	tests := []struct {
		...
		{"first_byte_timeout", TurnErrFirstByteTimeout, "供应商无响应时请稍后重试或切换模型；网络波动会自动重连"},
		{"provider_billing", TurnErrProviderBilling, "请在供应商控制台充值后重新发送"},
		{"provider_auth", TurnErrProviderAuth, "检查供应商 API Key 是否有效，或更换模型"},
