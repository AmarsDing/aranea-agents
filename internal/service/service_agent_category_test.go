...
import (
	...
	catv1 "aranea-agents/api/kratos/agent_category/v1"
	"aranea-agents/internal/biz"
	"aranea-agents/internal/service"
)

func TestToProtoCat(t *testing.T) {
	c := biz.AgentCategory{
		ID:           "cat-1",
		Key:          "general",
		Name:         "通用",
...
func TestFromProtoCat_Nil(t *testing.T) {
	got := service.FromProtoCat(nil)
	if got.ID != "" || got.Key != "" {
		t.Fatalf("expected zero-value AgentCategory, got %+v", got)
	}
}

func TestFromProtoCat_ToProtoCat_RoundTrip(t *testing.T) {
	pb := &catv1.AgentCategory{
		Id:           "cat-2",
		Key:          "dev",
		Name:         "开发",
		Description:  "开发分类",
	...
	bizCat := service.FromProtoCat(pb)
...
func TestToProtoTreeNode_Nil(t *testing.T) {
	if got := service.ToProtoTreeNode(nil); got != nil {
		t.Fatalf("expected nil, got %v", got)
	}
}

func TestToProtoTreeNode_Single(t *testing.T) {
	n := &biz.AgentCategoryTreeNode{
		Category: biz.AgentCategory{
			ID:   "cat-1",
			Key:  "root",
			Name: "Root",
		},
	...
	pb := service.ToProtoTreeNode(n)
	if pb.GetCategory() == nil || pb.GetCategory().GetId() != "cat-1" {
		t.Fatalf("category mismatch: %+v", pb.GetCategory())
	}
...
func TestToProtoTreeNode_WithChildren(t *testing.T) {
	n := &biz.AgentCategoryTreeNode{
		Category: biz.AgentCategory{
			ID:   "parent",
			Key:  "parent-key",
		...
		Children: []biz.AgentCategoryTreeNode{
			{
				Category: biz.AgentCategory{
					ID:   "child-1",
					Key:  "child-key-1",
	...
	pb := service.ToProtoTreeNode(n)
	if pb.GetCategory().GetId() != "parent" {
		t.Fatalf("parent id mismatch: %q", pb.GetCategory().GetId())
	}
	if len(pb.GetChildren()) != 2 {
		t.Fatalf("children count mismatch: %d", len(pb.GetChildren()))
	}
	if pb.GetChildren()[0].GetCategory().GetId() != "child-1" {
		t.Fatalf("child-1 id mismatch: %q", pb.GetChildren()[0].GetCategory().GetId())
	}
	if pb.GetChildren()[1].GetCategory().GetId() != "child-2" {
		t.Fatalf("child-2 id mismatch: %q", pb.GetChildren()[1].GetCategory().GetId())
	}
}

func TestToProtoTreeNode_DeepNesting(t *testing.T) {
	n := &biz.AgentCategoryTreeNode{
		Category: biz.AgentCategory{ID: "root", Key: "root", Name: "Root"},
		Children: []biz.AgentCategoryTreeNode{
			{
				Category: biz.AgentCategory{ID: "level1", Key: "l1", Name: "L1"},
				Children: []biz.AgentCategoryTreeNode{
					{
						Category: biz.AgentCategory{ID: "level2", Key: "l2", Name: "L2"},
					},
				},
			},
		},
	...
	l1 := pb.GetChildren()[0]
	l2 := l1.GetChildren()[0]
	if l2.GetCategory().GetId() != "level2" {
		t.Fatalf("deep nesting id mismatch: %q", l2.GetCategory().GetId())
	}
}

func TestToProtoTree_Empty(t *testing.T) {
	pb := service.ToProtoTree(nil)
	if pb == nil {
...
func TestToProtoTree_MultipleRoots(t *testing.T) {
	nodes := []biz.AgentCategoryTreeNode{
		{Category: biz.AgentCategory{ID: "r1", Key: "root1", Name: "Root 1"}},
		{Category: biz.AgentCategory{ID: "r2", Key: "root2", Name: "Root 2"}},
	}
	pb := service.ToProtoTree(nodes)
	if len(pb) != 2 {
		t.Fatalf("expected 2 roots, got %d", len(pb))
	}
	if pb[0].GetCategory().GetId() != "r1" || pb[1].GetCategory().GetId() != "r2" {
		t.Fatalf("root ids mismatch: r0=%q r1=%q", pb[0].GetCategory().GetId(), pb[1].GetCategory().GetId())
	}
}

func TestFromProtoCat_ZeroValues(t *testing.T) {
	pb := &catv1.AgentCategory{
		Id:      "cat-0",
		Key:     "zero",
		Enabled: false,
