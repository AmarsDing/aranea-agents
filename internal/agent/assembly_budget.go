...
// 管窗口溢出，本闸管绝对烧钱。513 万事故（轮数型放大，单轮峰值 86K 仅为
// 128K 窗口 67%、比例闸从未触线）证明：比例闸对「窗口远未触线但单次调用
// 已累积到 86K」结构性失明。（归因修订 2026-08-25：S02-armB/S08 初判
// 「单轮装配型爆炸」经 rounds-forensics.md 三方对账证伪，与 513 万同为
// 轮数型；本闸价值定位 = 兜底历史累积型单轮峰值 + soft 告警引导 evict。）
//
