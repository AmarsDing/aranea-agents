...
// newVoiceFastPathBeforeHook 在语音轮次把 GenerationConfig.ThinkingEnabled 显式
// 置 false（deepseek variant 映射为 {"thinking":{"type":"disabled"}}）；
// 非语音轮次保持 nil（provider 服务端默认）。LayerDynamic：标记来自 per-request ctx。
func newVoiceFastPathBeforeHook() callbacks.Callback {
	return callbacks.NewBeforeModelHook(4, callbacks.LayerDynamic, func(ctx context.Context, args *trpcmodel.BeforeModelArgs) (*trpcmodel.BeforeModelResult, error) {
		...
		if !VoiceFastPathFromContext(ctx) {
			return &trpcmodel.BeforeModelResult{Context: ctx}, nil
		}
		disabled := false
