...
func TestSentTextRecorder_Basic(t *testing.T) {
	rec := NewSentTextRecorder()
	target := DeliveryTarget{Channel: "telegram", Target: "ch1"}
	rec.Record(target, "hello")
	if !rec.Contains(target, "hello") {
		t.Fatal("expected to contain recorded message")
	}
	if rec.Contains(target, "other") {
		t.Fatal("should not contain unrecorded message")
	}
}

func TestSentTextRecorder_ContainsTarget(t *testing.T) {
	rec := NewSentTextRecorder()
	target := DeliveryTarget{Channel: "telegram", Target: "ch1"}
	rec.Record(target, "hello")
	if !rec.ContainsTarget(target) {
		t.Fatal("expected to contain target")
	}
	other := DeliveryTarget{Channel: "slack", Target: "ch2"}
	if rec.ContainsTarget(other) {
		t.Fatal("should not contain unrecorded target")
	}
}

func TestSentTextRecorder_NilRecorder(t *testing.T) {
	var rec *SentTextRecorder
	rec.Record(DeliveryTarget{Channel: "x", Target: "y"}, "hi")
	if rec.Contains(DeliveryTarget{Channel: "x", Target: "y"}, "hi") {
		t.Fatal("nil recorder should not contain")
	}
	if rec.ContainsTarget(DeliveryTarget{Channel: "x", Target: "y"}) {
		t.Fatal("nil recorder should not contain target")
	}
}

func TestSentTextRecorder_EmptyTarget(t *testing.T) {
	rec := NewSentTextRecorder()
	rec.Record(DeliveryTarget{Channel: "", Target: ""}, "hi")
	if rec.Contains(DeliveryTarget{Channel: "", Target: ""}, "hi") {
		t.Fatal("empty target should not be recorded")
	}
}

func TestSentTextRecorder_EmptyText(t *testing.T) {
	rec := NewSentTextRecorder()
	target := DeliveryTarget{Channel: "telegram", Target: "ch1"}
	rec.Record(target, "")
...
func TestWithSentTextRecorder_NilCtx(t *testing.T) {
	rec := NewSentTextRecorder()
	ctx := WithSentTextRecorder(nil, rec)
	if ctx == nil {
		t.Fatal("expected non-nil context")
	}
}

func TestWithSentTextRecorder_NilRecorder(t *testing.T) {
	ctx := context.Background()
	result := WithSentTextRecorder(ctx, nil)
	if result != ctx {
		t.Fatal("should return same context for nil recorder")
	}
}
