// Package mcpprobe evaluates MCP server rows (ported from pkg/backend PlatformService).
package mcpprobe
...
// TestResult matches legacy MCPServerTestResult JSON.
...
// Evaluate runs stdio-path or HTTP connectivity checks against config_json (legacy evaluateMCPServer).
