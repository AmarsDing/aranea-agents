...
| 失败模式（论文原话） | 丢失约束、错过中途信息、该问用户时瞎猜、跳过验证 | 我们的对策：每步 verify（M75 已有执行后验证闭环）+ 高危强制人审 + 注入防护（G2） |
| 威胁面 | Microsoft 官方文档明确警告 computer use 的 prompt injection 风险 | 运维场景威胁具象化：屏幕上的告警文本即不可信输入 |
...
| 层 | 机制 | 位置 |
|----|------|------|
| L1 检测 | 观察结果（元素名/文本）经**注入模式扫描**：指令性短语（"ignore previous instructions"/"忽略(以上|之前)指令"/"system prompt"/"you are now"等，中英双语、大小写不敏感）+ 可疑角色扮演前缀 | `biz/computeruse` 新增 `InjectionGuard` |
| L2 打标 | 命中即在该会话的观察结果中标记 `injection_suspected=true` + 审计记录（命中模式、元素 ref、片段摘要）——**证据可回溯** | Observe 编排 + AuditStore |
| L3 行为约束 | 命中会话内，后续写动作（act/launch）**强制升级为高危**（走逐次人工确认，复用 M75 danger 短路），只读动作放行 | Act/Launch 编排（与既有危险词策略并联） |
...
| 期 | 内容 | 形态 | 状态 |
|----|------|------|------|
| P0（本轮） | G2 注入防护（Go，TDD）；G3 评测集资产 + runner 骨架；G1 第 6 本 Skill 手册文本 | 代码 + 资产 + 文档 | 本方案配套开发文档 77 号 |
