/** Minimal agent/team shapes for routing utilities (decoupled from feature types). */
export type ChannelRoutingAgent = {
  id: string;
  agent_key: string;
  display_name: string;
  is_default: boolean;
...
export type ChannelRoutingTeam = {
  id: string;
  team_key: string;
  display_name: string;
};
...
export function channelAgentSelectOptions(agents: ChannelRoutingAgent[]): Array<{ label: string; value: string }> {
...
export function channelTeamSelectOptions(teams: ChannelRoutingTeam[]): Array<{ label: string; value: string }> {
...
export function resolveChannelAgentSelectValue(raw: string, agents: ChannelRoutingAgent[]): string {
...
export function pickDefaultAgentId(agents: ChannelRoutingAgent[]): string {
