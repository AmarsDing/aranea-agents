export default {
  ...
  catalogTab: {
    ...
    migrationHint: "Built-in legacy → models.dev id mapping (shipped with releases, not editable). Bindings are auto-migrated on sync; you can also align manually.",
    ...
    legacyModeBanner: "Legacy line-fragment format returned (not full JSON). Please restart the admin service and refresh.",
  ...
  channelEditor: {
    ...
    hints: {
      ...
      allowedUserIds: "Allowed user IDs (JSON array or comma-separated); leave empty for no restriction. Feishu: open_id (ou_xxx) or user_id",
      allowedGroupIds: "Allowed group chat_ids (Feishu oc_xxx), JSON or comma-separated; leave empty for no restriction. DMs are not affected",
  ...
  hooksPage: {
    ...
    subtitle: "Configure lifecycle callback rules for Agent, Model, Tool, and Runner events (log / notify / block / modify).",
    ...
    callbackEditor: {
      ...
      modifyPatchHint: "before_model: generation_config / append_system / append_user. before_tool: arguments full replace; merge_arguments deep merge (nested objects recursive, scalars/arrays from patch win).",
  ...
  webhooksPage: {
    ...
    subtitle: "Configure outbound webhook endpoints to push notifications to external services on Runner events (HMAC-SHA256 signature verification).",
