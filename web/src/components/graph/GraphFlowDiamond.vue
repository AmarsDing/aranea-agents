<template>
  <div
    :class="['graph-flow-diamond', `graph-flow-diamond--${data.nodeType}`, { 'graph-flow-diamond--selected': selected, 'graph-flow-diamond--running': data.execStatus === 'running', 'graph-flow-diamond--completed': data.execStatus === 'completed', 'graph-flow-diamond--failed': data.execStatus === 'error' || data.execStatus === 'failed', 'graph-flow-diamond--interrupted': data.execStatus === 'interrupted' }]"
    role="group"
  ...
  >
    ...
    <div class="graph-flow-diamond__inner">
      ...
      <q-badge v-if="data.execStatus && data.execStatus !== 'idle'" dense rounded class="graph-flow-diamond__status-badge" :class="diamondStatusBadgeClass">
        {{ data.execStatus === 'running' ? '运行' : data.execStatus === 'completed' ? '完成' : data.execStatus === 'failed' || data.execStatus === 'error' ? '失败' : data.execStatus === 'interrupted' ? '中断' : data.execStatus }}
      </q-badge>
    </div>
