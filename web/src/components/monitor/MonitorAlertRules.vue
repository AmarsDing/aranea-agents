<template>
  <q-card flat bordered class="monitor-card monitor-alert-rules">
    <q-card-section class="row items-center">
      ...
      <q-btn color="primary" unelevated rounded icon="save" label="保存" :loading="saving" @click="save" />
    </q-card-section>
    <q-separator />
    <q-card-section>
      ...
      <div v-for="(rule, idx) in rules" :key="rule.id || idx" class="monitor-alert-rule-row q-mb-md q-pa-md rounded-borders">
        <div class="row q-col-gutter-md">
          ...
          <q-input v-model.number="rule.threshold" class="col-6 col-md-2" dense outlined type="number" label="阈值" />
          <q-input v-model.number="rule.window_minutes" class="col-6 col-md-2" dense outlined type="number" label="窗口(分)" />
          <q-toggle v-model="rule.enabled" class="col-12" label="启用" />
          <q-input v-model="rule.notify_webhook_url" class="col-12 col-md-6" dense outlined label="Webhook URL" />
          <q-select
          ...
          />
          <q-input v-model.number="rule.cooldown_minutes" class="col-12 col-md-2" dense outlined type="number" label="冷却(分)" />
        </div>
      </div>
    </q-card-section>
  </q-card>
