  ...
  <q-dialog :model-value="open" persistent @update:model-value="$emit('update:open', $event)">
    <q-card class="app-dialog-card app-dialog-card--md app-glass-dialog">
      ...
      <q-card-section class="app-dialog-body q-gutter-md">
        <q-tabs :model-value="mode" dense align="left" class="text-primary" @update:model-value="$emit('update:mode', $event as 'form' | 'json')">
      ...
      <q-card-actions align="right" class="app-actions-bar app-glass-dialog__actions">
        ...
        <q-btn color="primary" rounded unelevated no-caps label="保存" :loading="saving" :disable="Boolean(configError)" @click="$emit('save')" />
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
