...
describe('deriveMemoryToolMode', () => {
  ...
  it('returns "working_memory" even with extra denied tools', () => {
    const deny = JSON.stringify(['memory_add', 'memory_update', 'memory_delete', 'memory_search', 'memory_load', 'some_other_tool'])
    expect(deriveMemoryToolMode(deny)).toBe('working_memory')
  })

  it('returns "framework_memory" when all working memory tools are denied', () => {
    const deny = JSON.stringify(['working_memory_read', 'working_memory_list', 'working_memory_write', 'working_memory_patch', 'working_memory_delete'])
