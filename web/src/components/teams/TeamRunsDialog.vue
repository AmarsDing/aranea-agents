  ...
  <q-dialog :model-value="modelValue" position="right" @update:model-value="$emit('update:modelValue', $event)">
    <q-card :class="['team-runs-panel', { 'is-dark': isDark }]">
      <q-card-section class="row items-center justify-between">
        <div>
          ...
          <div class="row items-center q-gutter-sm text-caption text-grey-7">
            ...
            <q-badge rounded :color="liveConnected ? 'positive' : 'grey'">{{ liveConnected ? "实时连接中" : "未连接" }}</q-badge>
          </div>
        </div>
      ...
      <q-card-section>
            ...
            <div class="run-detail q-pa-md">
              ...
              <div v-if="summariesByRun[run.id]" class="run-summary q-mb-md">
                ...
                <div class="text-body2">
                  成员 {{ summariesByRun[run.id]?.member_count }} · 工具调用 {{ summariesByRun[run.id]?.tool_call_count }} ·
                  in {{ summariesByRun[run.id]?.token_in }} / out {{ summariesByRun[run.id]?.token_out }}
                </div>
              </div>
              ...
              <div v-for="step in stepsByRun[run.id] || []" :key="step.id" class="run-step">
                ...
                <div class="text-caption text-grey-7">
                  {{ step.role || "worker" }} · {{ step.duration_ms }}ms · in {{ step.token_in }} / out {{ step.token_out }}
        ...
        <div v-if="!loading && runs.length === 0 && !error" class="text-center text-grey-7 q-pa-xl">暂无运行记录，请先进入 Chat 测试该 Team。</div>
      </q-card-section>
    </q-card>
  </q-dialog>
</template>
