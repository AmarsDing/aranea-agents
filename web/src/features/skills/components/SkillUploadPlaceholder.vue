      ...
      <q-card-section class="q-gutter-md">
        ...
        <div v-if="job">
          ...
          <div v-if="job.conflict_groups.length" class="q-mt-md q-gutter-md">
            <q-card v-for="group in job.conflict_groups" :key="group.group_id" flat bordered class="conflict-card">
              <q-card-section>
          ...
          <q-card v-if="refineResult" flat bordered class="q-mt-md">
            <q-card-section>
...
import { applySkillImport, getSkillImportJob, refineSkillConflictGroup, uploadSkillZip } from "../api";
import type { SkillConflictGroup, SkillImportDecision, SkillImportJob, SkillRefineResult, SkillSimilarityMetrics } from "../types";
...
async function startUpload() {
  ...
  try {
    const created = await uploadSkillZip(file.value);
    job.value = await getSkillImportJob(created.job_id);
...
async function refineGroup(groupId: string) {
  ...
  try {
    refineResult.value = await refineSkillConflictGroup(job.value.job_id, groupId, {});
...
async function applyImportResult() {
  ...
  error.value = "";
  try {
    await applySkillImport(job.value.job_id, decisions);
...
.skill-upload-card
  border-radius: 22px
  background: linear-gradient(135deg, rgba(127, 101, 73, 0.08), rgba(255, 255, 255, 0))
...
.metric-pill
  padding: 8px 10px
  border-radius: 12px
  background: rgba(127, 101, 73, 0.08)
