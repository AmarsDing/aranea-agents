...
export function useProviderWizard(deps: {
  ...
  function applyModelPresetValues(preset: ProviderModelPreset, overwrite = false) {
    ...
    f.context_window_k = overwrite || !f.context_window_k ? preset.contextWindowK ?? f.context_window_k : f.context_window_k;
    f.max_output_tokens = overwrite || !f.max_output_tokens ? preset.maxOutputTokens ?? f.max_output_tokens : f.max_output_tokens;
  ...
  const currentAuthType = computed(() => {
    ...
    return presets.currentProviderPreset.value?.authType || runtimeProfileFor(providerForm.provider_code).authType || "api_key";
  });
