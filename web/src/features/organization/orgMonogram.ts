 ...
 * 组织 monogram 工具函数
 *
 * 取公司 key 的大写首字母作为"icon"视觉锚点，
 * 颜色由 key 字符的简单 hash 映射到组织色板。
 * 共享于 OrgCard / OrgDrawer / OrgTableRow。
 */
...
export function monoBgForKey(key: string): string {
  ...
  const s = getComputedStyle(document.documentElement).getPropertyValue(`--palette-org-${idx}-start`).trim();
  const e = getComputedStyle(document.documentElement).getPropertyValue(`--palette-org-${idx}-end`).trim();
