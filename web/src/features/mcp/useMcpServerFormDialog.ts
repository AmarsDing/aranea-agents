...
export function useMcpServerFormDialog(
  modelValue: () => boolean,
  ...
  function resetForm() {
    serverError.value = "";
    const current = row();
    const config = parseJSON<McpServerConfig>(current?.config_json, {});
    Object.assign(form, {
      name: current?.key || "",
  ...
  async function persist(options: { close: boolean; notify: boolean }) {
    ...
    try {
      const payload = buildPayload();
      const saved = row()
        ? await mcpStore.editServer(row()!.id, payload)
        : await mcpStore.addServer(payload);
      emit("saved", saved);
  ...
  function buildPayload(): PlatformResourceInput {
    const existingMetadata = parseJSON<McpServerMetadata>(row()?.metadata_json, {});
