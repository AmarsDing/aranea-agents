  ...
  <q-dialog :model-value="modelValue" persistent @update:model-value="$emit('update:modelValue', $event)">
    <q-card style="min-width: 480px; max-width: 96vw">
      ...
      <q-card-section>
        ...
        <q-form class="row q-col-gutter-sm" @submit.prevent="save">
          <q-input v-model="form.credential_key" class="col-12 col-md-5" dense outlined label="凭据键" hint="通常为 Authorization 或 API 头名" />
          <q-input v-model="form.secret" class="col-12 col-md" dense outlined type="password" label="密钥 / Token" />
          <div class="col-12">
            <q-btn color="primary" unelevated rounded label="保存凭据" type="submit" :loading="saving" :disable="!canSave" />
          </div>
        </q-form>
      </q-card-section>
    </q-card>
