...
describe('chat model capabilities', () => {
  ...
  it('lets explicit vision capability override provider heuristics', () => {
    expect(modelSupportsImageInput({ provider: 'deepseek', model: 'vision', capabilities: { vision: true } })).toBe(
      true,
    );
  });
  ...
  it('honors explicit non-vision capability metadata', () => {
    ...
    expect(
      modelSupportsImageInput({ provider: 'other', model: 'text', capabilities: { vision: true, text_only: true } }),
    ).toBe(false);
  });
