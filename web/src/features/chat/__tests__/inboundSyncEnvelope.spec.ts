...
describe('inboundSyncEnvelope (DECO-01 / M55-SYNC)', () => {
  it('reads session_revision from envelope root or metadata', () => {
    ...
    expect(envelopeSessionRevision(env({ metadata: { session_revision: 5 } }))).toBe(5);
  ...
  it('reads channel source for cross-plane routing', () => {
    ...
    expect(envelopeSource(env({ metadata: { source: 'channel' } }))).toBe('channel');
  });
  ...
  it('runner_completion always completes turn', () => {
    expect(isTurnCompleteEnvelope(env({ type: 'runner_completion' }))).toBe(true);
  });
