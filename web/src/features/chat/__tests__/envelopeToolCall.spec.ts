...
import { activityMessageId, cancelRunningToolMessages, envelopeToToolEvent, mergeToolEvents, upsertToolMessage } from "../envelopeToolCall";
...
describe("envelopeToolCall", () => {
  ...
  it("cancels running tool cards on stop", () => {
    const sessionId = "sess-1";
    const running = upsertToolMessage([], sessionId, {
      id: "env-1",
      type: "tool_call",
      author: "agent-a",
      session_id: sessionId,
