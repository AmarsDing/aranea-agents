...
export function classifyActivityKind(toolName: string): ActivityKind {
  ...
  if (["mcp_call", "mcp_list_tools", "mcp_list_servers", "mcp_inspect_tools"].includes(name) || name.startsWith("mcp:") || name.startsWith("mcp_")) {
    return "mcp";
  }
...
export function resolveDisplayLabel(event: ToolUseEvent): string {
  return event.display_label?.trim() || event.tool_label?.trim() || builtinLabels[event.tool_name] || event.tool_name || "tool";
}
