...
export function useChannelEditorForm(
  props: EditorProps,
  ...
  const selectedRoutingAgent = computed(() => routingAgents.value.find((a) => a.id === defaultAgentId.value) ?? null);
  const {
  ...
  function jsonError(value: string) {
    try {
      ...
      return err instanceof Error ? err.message : t('channelEditor.jsonFormatError');
    }
  }
  ...
  const canSave = computed(() => {
    const base = Boolean(
      form.key.trim() && form.name.trim() && selectedType.value && !configError.value && !metadataError.value,
    );
  ...
  const webhookPreview = computed(() => {
    ...
    const normalized = path
      ? path.startsWith('/')
        ...
        : `/${path}`
      : form.key.trim()
        ? `/webhooks/${form.key.trim()}`
        : '';
    if (!normalized) return '';
    return buildChannelWebhookURL(normalized, { public_webhook_origin: publicWebhookOrigin.value });
  });
  const webhookIsLocalhost = computed(() =>
    isLocalhostOrigin(publicWebhookOrigin.value || (typeof window !== 'undefined' ? window.location.origin : '')),
  );
  ...
  function readField(field: ChannelPlatformField): string {
    ...
    switch (bind.source) {
      case 'form':
        return String(form[bind.key] ?? '');
      case 'credential':
        return credentialDraft[bind.key] ?? '';
      case 'config':
        return configDraft[bind.key] ?? '';
      case 'feishu':
        return bind.key === 'app_id' ? feishuAppId.value : feishuRegion.value;
      case 'routing':
  ...
  function writeField(field: ChannelPlatformField, value: string) {
    ...
    switch (bind.source) {
      case 'form':
        ...
        break;
      case 'credential':
        credentialDraft[bind.key] = value;
        break;
      case 'config':
        configDraft[bind.key] = value;
        break;
      case 'feishu':
  ...
  function writeFieldBool(field: ChannelPlatformField, value: boolean) {
    if (field.bind.source === 'form' && field.bind.key === 'enabled') {
      form.enabled = value;
      return;
    }
  ...
  function resolveLongTaskPreset(cfg: ChannelConfig, metadata: ChannelMetadata) {
    ...
    selectedLongTaskPreset.value = inferLongTaskPresetId(cfg.receive_mode || receiveMode.value, cfg.config || {});
  }
  ...
  async function saveAndTest() {
    ...
    try {
      ...
      $q.notify({
        type: 'negative',
        message: err instanceof Error ? err.message : t('channelEditor.saveOrTestFailed'),
      });
    } finally {
      testing.value = false;
    }
