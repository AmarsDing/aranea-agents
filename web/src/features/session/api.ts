...
function kratosSessionToLegacy(s: KratosSession): Session {
  ...
  return {
    ...
    status: (s.status ?? 'idle') as SessionStatus,
    status_reason: (s.statusReason ?? '') as SessionStatusReason,
    status_changed_at: s.statusChangedAt ?? '',
...
export async function listSessionTurns(
  ...
  limit = 20,
  offset = 0,
): Promise<{ items: SessionTurn[]; total: number }> {
