# Aranea 真机测试公共库
# 用法: . .\..\_lib.ps1 后调用 Api-Get / Api-Post / Api-Send / Record
$script:BaseUrl = "http://localhost:8810"
...
$script:TestRoot = "f:\myproject\aranea-agents\docs\testing\realmachine-20260817"
...
# Api-Call -Method GET -Path "/v1/agents" -Body obj/file -OutFile path
# 返回 [pscustomobject]@{ Code; Ms; Body; Error }
function Api-Call {
    param([string]$Method = "GET", [Parameter(Mandatory)][string]$Path, $Body = $null, [string]$OutFile = $null, [int]$TimeoutSec = 60)
    $tok = Get-Token
    $sw = [Diagnostics.Stopwatch]::StartNew()
    $curlArgs = @("-s", "-o", "-", "-w", "`n%{http_code}", "-m", "$TimeoutSec", "-X", $Method, "-H", "Authorization: Bearer $tok", "-H", "Content-Type: application/json")
    if ($null -ne $Body) {
        $bf = $Body
    ...
    $out = & curl.exe @curlArgs "$($script:BaseUrl)$Path" 2>$null
    $sw.Stop()
...
function Api-Post { param([string]$Path, $Body, [string]$OutFile, [int]$TimeoutSec = 120) Api-Call -Method POST -Path $Path -Body $Body -OutFile $OutFile -TimeoutSec $TimeoutSec }
...
# Record -Module "01" -Id "AGENT-01" -Name "..." -Result PASS -Detail "..." -Ms 123
function Record {
