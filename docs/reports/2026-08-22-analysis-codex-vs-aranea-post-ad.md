...
F1–F4 后再估：Codex **8.6** / Aranea **8.5**（差 **0.1**）。  
平台分不变：Codex ~3.5 / Aranea ~8.5（不加权）。

读法：合同层、执行回路、增量 world state、OS 进程围栏、MCP 脏刷新、默认 ACP argv 与 §7 金丝雀已补上。F 轮补了 shell 行数、失败再确认、仓库技能目录、coding 面更早 broker。剩下的硬差距在 **完整 OS token/Seatbelt 档**、真实编码桥二进制冒烟 / M3 管理页、MCP templates/subscribe。
...
| Exec | 9.0 | 7.5 | **8.5** | D2 路径沙箱；**E1** 安全 shell 免确认；**E4** `duration_ms`；**E5** 禁 `save_file` 覆盖；**E3** Job Object / bwrap；**F1** 回执始终 `total_lines`；**F2** 失败后再确认 | 无 Seatbelt / 完整 FS+网络策略；无 execpolicy 落盘 / 命令 hash `ApprovedForSession` |
| Discovery | 8.5 | 7.0 | **8.3** | B3 BM25；B4 `tool_hints`；MCP **broker**；**F4** coding/spirit 无 `mcp:` allow 时 1 个直连工具即降级 broker | 直连 `mcp_tool_set` 并未默认 Deferred（和 Codex defer 不是同一条路） |
| Skill | 8.0 | 7.5 | **8.3** | B5 `$slug`；A4 `body_too_long`；**F3** 仓库技能 cue + `skill_load` overlay | 不入库、不替代平台 catalog；无 per-workspace skill watch |
...
加权（15/15/15/10/10/10/10/15）：A–D 后 ≈ **7.8**；E1–E6 ≈ **8.1**；E7–E10 ≈ **8.3**；E3+热刷新后再估 ≈ **8.4**；F1–F4 后再估 ≈ **8.5**。
...
因此 `go test` 的路径是：**coding 专项下安全命令直接跑**（E1：`go test` / `go vet` / `git status|diff|log` / `rg` / `ls|dir` / linter 免第一张卡；`rm` / `git push` 等高危仍确认；未知命令保持确认）。同一 invocation 里这条（或任意 shell）非 0 / 启动失败后，下一条安全命令也要确认（F2）；session/persisted grant 仍可跳过。不是禁脚本。和 Codex 仍差 execpolicy 落盘与完整 OS token。
...
**Shell 回执**：`hostexec.mapExecResult` / `mapPollResult` 现有 `exit_code` + 前台 **`duration_ms`（E4）** + **`total_lines`（F1，截断前行数）**；长任务仍有 `running_for_ms`。decorator 截断 / offload 信封会抬升 `exit_code` / `duration_ms` / 原始 `total_lines`。
...
| HITL 范围 | `request_permissions.rs`：`Turn` / `Session`（**无全局**）；`ApprovedForSession` 按命令 hash / patch 路径缓存；可走 Guardian | D1 `allow_always` 仅本编码任务。内部 `shell_exec` 有 session grant + F2 失败再问；仍缺 execpolicy 补丁落盘 / 命令 hash |
| 权限是工具 | `RequestPermissionsHandler`：模型可申请扩权，结果 merge 进 `PermissionProfile` | Aranea 权限是 profile 推断 + 确认门，模型不能在回合内申请「加写某个根」 |
| 回执三件套 | `format_exec_output_for_model`：`Exit code` / `Wall time` / `Total output lines` | A3 截断信封 + F1 shell 回执始终带 `total_lines`；`exit_code` / `duration_ms` 已有 |
...
| F1 | P1 | shell 回执始终带 `total_lines`；截断信封抬升 exit/duration/行数 | ✅ 2026-08-22 |
| F2 | P1 | 同一 invocation 里 shell 失败后再确认（含截断信封里的 exit_code） | ✅ 2026-08-22 |
| F3 | P1 | 仓库 `.agents/skills` cue + `skill_load` / `$slug` overlay | ✅ 2026-08-22 |
| F4 | P2 | coding/spirit 无 `mcp:` allow 时直连即降级 broker（有 allow 则 ≥8） | ✅ 2026-08-22 |
...
F1–F4 把回执行数、失败再确认、仓库技能目录和 coding 面 schema 预算补上。  
