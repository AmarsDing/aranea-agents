...
> 日期：2026-08-14
> 类型：调研综合 + 实施计划
> 状态：✅ 全量完成（Batch-1 ~ Batch-6 于 2026-08-14 全部通过门禁；Batch-0 为 P0 收尾批次，其修复已在 P1 门禁中覆盖验证）
...
| ADR-E | 评测态 profile 钉住语义与互斥优先级（P3-4） | P3 |
...
| Batch-6 ✅ | P3-3 + P3-4（架构级，先 ADR 评审） | team/agent 全量绿（team 0.66s / agent 27.5s）+ build 绿（2026-08-14 通过）；P3-3 TeamRun 图执行挂起/唤醒（SuspendIdleWaits 内存 evict+DB 保留 / ensureSessionResident 按需重建 / CleanupStaleSessions 改按 LastActivityAt 判龄，cleanup ticker 同循环驱动，ADR-D 经代码深潜修正为协调器会话维度），P3-4 评测态 profile（EvalProfileDef 定义 JSON + PinnedModelSelector fail-loud 钉住全成员模型 + WithModelRequestExtraFields 钉 seed/temperature + WithToolFilter 工具白名单，与 model_cascade 互斥 eval 胜出，FlowLog `team.eval_profile.applied` 审计）。ADR 落盘：[ADR-D](./2026-08-14-review-adr-member-session-suspend.md)、[ADR-E](./2026-08-14-review-adr-eval-profile.md)；新增 model_selector_eval_test（4 用例）+ team_eval_profile_test（10 用例） |
