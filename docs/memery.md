导读：本文讲述作者对 Memory 的一些理解与思考。（文章内容基于作者个人技术实践与独立思考，旨在分享经验，仅代表个人观点。）
...
**原文与配图**：本文若在头条等平台发布，可对照原文插图阅读——[今日头条文章（同主题 DeepLink）](https://www.toutiao.com/article/7636596749927039530/)（正式配图以客户端/网页加载为准）。自动化抓取普遍无法读取头条正文与图片 CDN，因此以下在 **OCR 误识别「图片」处已改回语义**，并用 **SVG / Mermaid** 补全流程示意，便于离线阅读。
...
1、核心命题：Memory 在 Agent 系统里到底"是什么"
...
2、小结：为什么Raw Ledger+views+policy是自然解
...
1）Memory 需要以 event/Action 序列为第一性对象：否则
provenance/rollback/replay 无从谈起。
...
从结构分解看：整体可写成组合形式 **输出 = Agent(观测, Memory(历史))**，其中 **Agent** 表示通用 LLM/Agent（推理、规划、工具调用），**Memory** 表示记忆系统（写入、索引、检索、过滤、摘要、时序化）把历史映射为可注入的证据上下文。这个接口意味着：你可以在 **不改变 Agent** 的前提下，通过改进 Memory 获得增益；也可以在 **不改变 Memory** 的前提下，通过提升 Agent 获得增益。


从优化视角看：若把整体指标记为 **J(Agent, Memory)**（成功率/满意度/长期回报等），当它允许「分块优化」——即 **固定 Agent、只调 Memory** 仍能稳定提升，且 **固定 Memory、只调 Agent** 也能提升——则说明两者存在可用的弱耦合结构；这也是我们更偏好把记忆能力以 System 2 工具化/外置化实现，而不是强行内化进 System 1 权重。
...
同一分工的 Mermaid 示意（可与上图 ASCII 对照阅读；需支持 Mermaid 的 Markdown 预览）：
...
我们认为，只需要维护一个动态的非参数化经验库（用在推理时：每步决策/生成动作前检索与当前状态相似的历史轨迹/样本，估计**优势等相关量**并对 logits 做加性调制；维护方式：每个任务/episode 结束将新轨迹 (s,a,r,G) 写回入库，按质量/时效去重淘汰并更新检索索引），即可实现梯度下降类似的效果。逼近的是fine-tune的效果。
...
2、Memory System 控制层/策略层（policy）
...
2）InfMem：PreThink-Retrieve-Write 协议
...
具体来说，Zep 会对图谱中的边（Edge）加入时间标注（Temporal Validity）。

例如：「甲公司 —任职→ 张三」不再是一个没有时间界的静态事实，而是带有一个**有效时间窗口**（valid_time 区间）：只在某段时间内成立，用以区分「曾经为真」与「当前仍为真」。


![Temporal validity 示意图](assets/memery/temporal-validity.svg)
...
1）时序记忆（Temporal Memory）深度思考：从"发生过什么"到"何时为真、何时被相信"
...
2）时序记忆的本质命题：三视角交叉（Episodic × Bi-temporal × TKG）
...
Zep/Graphiti 的 TKG 核心是给边/事实加入 Temporal Validity（见图：有效时间窗与 query_time 的关系），检索时按查询的时间语境合成"当前真值图谱"。这本质上是把"曾经为真"与"当前为真"结构化分离，而不是交给 LLM 去"记得别搞混"。
...
另外，MAGMA 论文（2025）提出了四正交图架构（语义、时间、因果、实体），其中时间图的设计也值得参考。MAGMA 将**时间图**建为**严格有序的时间边**所组成的**不可变时间骨干**；快速路径只建时间边，慢速路径异步推断因果/实体等高阶关系。消融实验显示移除时间骨干后评判分数从 0.700 降至 0.647，证明时间排序提供了不可替代的推理轴。
...
policy 层引入"触发再验证"：当 query_time 远离 last_verified，先走小步试探/验证路径，再提高该 skill 在排序中的权重并将其纳入可执行候选集。
...
Δ 修正项中的时序分量：时间如何进入 **Δ**（超越「仅做时间衰减」）


当前机制是：检索相似历史轨迹/样本、估计**优势**并对 logits 加性调制。时序进入 Δ，不应仅是给旧样本乘一个 decay；更结构化的进入方式是"先限定候选，再调制优势"。
...
让**优势估计 / Δ 的组合模块**看到"时间结构特征"，而非只看到"老不老"：对同一 fact_key（或同一类经验轨迹）可抽取结构化时间信号进入优势估计：
...
4、程序性层（The Procedural Layer）：从"知识"到"技能"
...
第一段是"生成"：ProcMEM 引入语义梯度（Semantic Gradient）。它不是对模型参数做自动微分，而是对技能的三部分（触发/执行/终止）做事后归因，总结"这条技能为什么在这段轨迹里导致成功或失败"，并把更新方向以自然语言建议的形式产出。对一批轨迹的语义梯度做聚合后，就得到一个更稳定的批级更新信号，用来由**旧技能变体生成候选技能变体**（可以理解为一种"非参数化的梯度上升"，但更新对象是技能文本结构，而不是权重）。
...
5、Memory 整合层（The Integration Layer）：潜层融合与零样本对齐（Memory Tokens）
...
1）LycheeMemory：潜层压缩与 KV-Cache 注入
...
2）文件系统（File System / Storage Plane）


以"权威记录（Raw Ledger，记忆原始数据/raw账本）+ 派生视图（views）"为核心的数据层。底层需要承载时序一致性与冲突消解（事实在时间上何时为真），上层需要做语义压缩与分层固化（把大量事件/片段逐步抽象成更高层的表征），并且始终保留可溯源链路（这类方向可分别类比时序 KG 与递归固化/分层记忆）。

3）可执行文件（Executable / Skill Plane）

把经验固化成可执行、可复用的程序性单元（skill/macro/workflow），让系统复用"怎么做"，而不仅是复用"是什么"。关键不是技能形式本身，而是它必须满足可执行、可验证、可治理，才能长期演化且不发生系统性退化。

4）总线接口（Interface / Context Bridge）

记忆与推理之间的接口层。它负责把外部状态以尽量低开销、低失真、可控的方式注入到计算核心中，并支持可观测与溯源（部分工作用 latent token / memory token 作为桥接手段）。

5）学习引擎（Learning Engine / Online Adaptation）

在不必频繁更新权重的前提下，持续把交互反馈转化为可用的改进信号。它可以体现在推理端的优势调制、技能层的演化算子、检索策略的分块优化等。关键是让"学习"发生在外部状态与策略上，从而保持可插拔、可回滚、可 A/B 的工程属性（JitRL/优势调制是其中一种实现方式）。
