...
**作为** Agent 开发者，**我希望** 为 Agent 配置发布质量门禁（关联数据集、指标、最低分与最大跌幅），**以便** skill 发布与 pack 安装前自动跑回归，分数不达标时阻断变更并收到通知。

**验收标准**：
- 可配置门禁：开关、关联 Agent、关联数据集、指标、最低分、最大跌幅
- skill 发布与 pack 安装前自动对配置的数据集同步跑一次回归
- 分数低于最低分，或相对最近一次已完成运行的基线跌幅超过阈值时，阻断发布并发出通知
- 门禁未启用或不可用时不影响正常发布流程
...
15. 发布质量门禁可配置，skill 发布 / pack 安装前自动回归，低于阈值阻断并通知（US-13）
