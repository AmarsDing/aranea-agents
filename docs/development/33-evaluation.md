...
- 可选择数据集和 Agent 启动评估
- 评估异步执行，不阻塞调用方
- 可查看评估运行状态（pending → running → completed/failed/cancelled）
- 可取消进行中的运行（终态 cancelled，保留已落库结果）
...
**验收标准**：
- 可按 Agent 配置门禁（无覆盖则回落到平台默认）：开关、数据集、指标、最低分、最大跌幅、模式
- 模式 `advisory`（默认）：发布不阻塞，回归后台跑完后通知
- 模式 `blocking`：发布等待回归，阈值不达标返回冲突并拒绝
- 配置了最大跌幅但无已完成基线时，两种模式都硬阻断并启动基线评估
- 门禁未启用或不可用时不影响正常发布流程
...
17. 同一数据集可对多个 Agent / 模型 / 工具配置开实验矩阵（可选提示词覆盖）；无检索时 faithfulness / context_precision 记 N/A；可钉住历史版本重跑；judge token 入账
18. 失败用例如写入 Learning Loop；结果可跳监控 Trace
19. 在线评估采样率可配、趋势按触发来源拆分、分数连跌触发系统通知（US-14）
20. 失败用例按错误信息归组可查（US-15）
21. A/B 对比可人工裁决更优并沉淀偏好记录（US-16）
22. 红队用例包可导入并输出攻击成功率（US-17）
23. 运行记录数据集 hash，对比时数据集变更给出提示（US-18）
24. 可列出/编辑/删除单条用例（P0 运营闭环）
25. 可取消进行中的评估运行（终态 cancelled）
