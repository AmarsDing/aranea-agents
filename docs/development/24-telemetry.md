...
本文档描述 Aranea 可观测性（Telemetry）的产品边界、四轨架构与验收口径。技术方案见 [`24 telemetry.design.md`](./24%20telemetry.design.md)；实现差距与任务以 [`24-telemetry-development.md`](./24-telemetry-development.md) 为准。
...
| Jaeger 里能否看到 LLM/Tool 细粒度 Span？ | OTel 自定义 Span | `turntrace.Bridge` → `llm.call` / `tool.call` 子 Span | ✅ |
| Graph / Team 执行是否有 OTel Span？ | 分布式 Trace | `graph.execute` / `team.run` 根 Span + 子 Span | ✅ |
| 高负载下 Trace 采样如何控制？ | Sampler | `OTEL_TRACES_SAMPLER` / `OTEL_TRACES_SAMPLER_ARG` | ✅ HTTP / ❌ gRPC |
| Langfuse Trace 是否可导出？ | Langfuse | `internal/telemetry/langfuse.go` → trpc `telemetry/langfuse` | ✅（需配置 Langfuse） |
...
| **D — Langfuse 导出** | `internal/telemetry/langfuse.go` | LLM Trace 导出至 Langfuse 平台 | `conf.Bootstrap.Langfuse` |
...
- Usage spans：`TraceEmitter.MetadataJSON()` → `recordTurnUsage`（含 `otel_trace_id` / `otel_root_span_id`）。
- OTel：`turntrace.Bridge` 管理 `chat.turn` / `team.run` / `graph.execute` 根 Span + `llm.call` / `tool.call` 子 Span。

### 2.4 Langfuse（轨道 D）

- 配置：`conf.Bootstrap.Langfuse`（Enable / PublicKey / SecretKey / BaseUrl）。
- 实现：`internal/telemetry/langfuse.go` → trpc `telemetry/langfuse.Start`。
- 生命周期：Wire 注入 `LangfuseRuntime`，独立于 OTLP shutdown。
...
- [x] Jaeger 可见 LLM 调用、工具调用等细粒度 OTel Span（`llm.call` / `tool.call`）。
