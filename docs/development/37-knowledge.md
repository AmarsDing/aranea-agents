...
- **页面结构**：知识库页由「文档/检索/设置」三 Tab 重做为「文档（资源管理器） | 图谱 | 检索 | 设置」（图谱二期）。
- **文档 Tab**：三栏（Vault 树 | 目录内容 | 详情面板），顶部统一搜索框 + 面包屑，底部常驻拖拽上传区（落入当前目录）。
- **搜索框**：即时区（文件名，输入即出，150ms 防抖）+ 内容区（回车触发，结果按文件分组、命中高亮）。
- **详情面板**：摘要卡（摘要/标签/类型/来源血缘）+ 关联区（出链/入链/相关推荐，标注来源类型）+ Markdown 预览。
