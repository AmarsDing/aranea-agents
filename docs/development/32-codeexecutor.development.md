...
> **版本**：2026-06-17 | **状态**：🟢 P0–P2 + Review 修复已落地；❌ Jupyter / Workspace / Interactive（P3）
> **需求**：[32-codeexecutor.md](./32-codeexecutor.md) · **设计**：[32-codeexecutor.design.md](./32-codeexecutor.design.md)
...
> 架构设计与接口契约见 [32-codeexecutor.design.md](./32-codeexecutor.design.md)。
...
| 配置 | `internal/agent/codeexecutor/config.go` | EnvConfig + ValidTypes + ResolveType |
| 能力探测 | `internal/agent/codeexecutor/capabilities.go` | Capabilities / IsBackendAvailable |
| Docker 探测 | `internal/agent/codeexecutor/docker_probe.go` | DockerAvailable / ResetDockerProbe |
| 运行时回退 | `internal/agent/codeexecutor/docker_fallback.go` | dockerRuntimeFallback |
| 指标装饰器 | `internal/agent/codeexecutor/metrics_executor.go` | Prometheus + blocks_total |
| Docker 适配 | `internal/agent/codeexecutor/docker_adapter.go` | DockerExecutor → 框架 CodeExecutor |
| 输出文件 | `internal/agent/codeexecutor/output_files.go` | CollectOutputDirFiles |
| 语言映射 | `internal/agent/codeexecutor/language.go` | languageRuntime |
| 项目 Executor | `internal/agent/codeexecutor/executor.go` | DockerExecutor + 项目 LocalExecutor（非 Skill 主路径） |
| Container 后端 | `internal/agent/codeexecutor/container_linux.go` / `container_stub.go` | build tag `codeexec_container` |
...
| biz 校验 | `internal/biz/code_executor.go` | ValidateCodeExecutorType |
| biz 类型 | `internal/biz/agent_types.go` | CodeExecutorType + GetCodeExecutor |
| biz 默认值 | `internal/biz/agent_defaults.go` | CodeExecutorType: "local" |
| Service | `internal/service/monitor_code_executor.go` | CodeExecutorService |
| Ent Schema | `internal/data/ent/schema/agent_runtime_setting.go` | code_executor_type 列 |
| Wire | `cmd/admin/wire.go` | provideCodeExecutorFactory |
| Proto (Monitor) | `api/kratos/monitor/v1/monitor.proto` | GetCodeExecutorCapabilities RPC |
| Proto (Agent) | `api/kratos/agent/v1/agent.proto` | code_executor_type field 101 |
| 前端 | `web/src/pages/agent-settings/AgentSettingsSkillsTab.vue` | 执行器选择 + capabilities 提示 |
...
| Docker 适配层 | ✅ | `dockerAdapter` → 框架 `CodeExecutor` |
...
| Factory（Wire 单例） | ✅ | `internal/agent/codeexecutor/factory.go`（Wire 单例 + lazy E2B/Container） |
| Docker 不可用回退 | ✅ | probe 回退 + 运行时 `dockerRuntimeFallback` |
| 生产环境拒绝 local | ✅ | `applyAvailabilityFallback` → `TypeDisabled`（`AllowLocalInProd` 可关闭） |
| Prometheus 全路径 | ✅ | metrics 装饰器 + blocks_total + exit code → error |
| E2B 沙箱 | 🟡 | lazy 注册；需 `E2B_API_KEY`；Key 缺失时 fallback local |
| Container 执行器 | 🟡 | `codeexec_container` build tag；默认 stub |
| 执行器 capabilities API | ✅ | `GET /v1/monitor/code-executor-capabilities` |
| biz 类型校验 | ✅ | `ValidateCodeExecutorType` → API 400 |
| 前端执行器选择 | ✅ | `AgentSettingsSkillsTab.vue` + capabilities 禁用不可用项 |
...
### Phase 1：Agent 配置 + Factory + 观测（P1）✅

**目标**：Agent 粒度选择执行器；Factory 替代环境变量硬编码；metrics 全覆盖。
...
| 1.9 | Docker 不可用 → local 回退 | `factory.go` + `docker_fallback.go` + `docker_probe.go` | ✅ |
| 1.10 | 前端 Agent 设置：执行器类型 | `web/.../AgentSettingsSkillsTab.vue` | ✅ |
| 1.11 | 测试 | `factory_test.go` · `code_executor_test.go` · `executor_test.go` · `config_test.go` | ✅ |
...
| 2.2 | Factory lazy 注册框架 `container.CodeExecutor` | `factory.go` + `container_linux.go` + `container_stub.go` | ✅ |
...
| `internal/agent/codeexecutor/config.go` | ✅ | EnvConfig + ValidTypes + ResolveType |
| `internal/agent/codeexecutor/capabilities.go` | ✅ | Capabilities / IsBackendAvailable |
| `internal/agent/codeexecutor/docker_probe.go` | ✅ | DockerAvailable / ResetDockerProbe |
...
| `internal/agent/codeexecutor/language.go` | ✅ | languageRuntime |
| `internal/agent/codeexecutor/executor.go` | ✅ | 项目 Executor + Local/Docker 底层 |
| `internal/agent/codeexecutor/container_linux.go` | ✅ | Container 后端（build tag `codeexec_container`） |
| `internal/agent/codeexecutor/container_stub.go` | ✅ | Container 后端 stub（默认） |
| `internal/agent/codeexecutor/factory_test.go` | ✅ | Factory 测试 |
| `internal/agent/codeexecutor/executor_test.go` | ✅ | Executor 测试 |
| `internal/agent/codeexecutor/config_test.go` | ✅ | Config 测试 |
...
| `internal/biz/code_executor_test.go` | ✅ | biz 校验测试 |
| `internal/biz/agent_types.go` | ✅ | CodeExecutorType + GetCodeExecutor |
| `internal/biz/agent_defaults.go` | ✅ | CodeExecutorType 默认值 |
| `internal/service/monitor_code_executor.go` | ✅ | CodeExecutorService |
| `internal/data/ent/schema/agent_runtime_setting.go` | ✅ | code_executor_type 列 |
...
| 文件 | 状态 | 说明 |
|------|------|------|
| `api/kratos/monitor/v1/monitor.proto` | ✅ | GetCodeExecutorCapabilities RPC + CodeExecutorCapability message |
| `api/kratos/agent/v1/agent.proto` | ✅ | code_executor_type field 101 |

### 8.3 前端

| 文件 | 状态 | 说明 |
|------|------|------|
| `web/src/pages/agent-settings/AgentSettingsSkillsTab.vue` | ✅ | 执行器选择 + capabilities 提示 |
