# Memory 记忆 — 产品需求（总）
> **本文定位**：记忆模块 **跨层产品需求** 与 **Memory Center UX**。分层规格见 [`L0.md`](./L0.md)～[`L4.md`](./L4.md)；设计见 [`memory.design.md`](./memory.design.md)；进度见 [`memory-development.md`](./memory-development.md)。
>
> **理论参考**（非验收标准）：[`theory.md`](./theory.md)

---

本文档从**用户角度**设计 aranea 的统一记忆管理界面，把 L0～L4 分层需求中的技术能力，整理成可理解、可审计、可控制的产品体验。
...
> 完整的组件文件结构、TypeScript 类型定义、API 函数签名、路由配置见 [`memory.design.md`](./memory.design.md) 附录 A §九。
...
> 具体的 Proto 定义和 TypeScript API 函数签名见 [`memory.design.md`](./memory.design.md) 附录 A §二/§九。
...
|------|---------------|
| 上下文窗口 | L0 快照列表、L0 预览 |
| 工作记忆 | L1 任务列表、字段列表、字段历史 |
| 事件时间线 | L2 事件列表 |
| Episodes | L2 episode 列表、巩固 |
| Marks | L2 标记列表 |
| Facts | L3 fact CRUD、confirm/reject |
| Conflicts | L3 冲突列表、解决 |
| L3 Recall | L3 语义检索 |
| Graph | L4 实体/关系/邻居查询 |
| Evolution | L4 身份/策略/提议/事件 |
| Settings | Agent 运行时设置更新 |
...
## 21. 记忆中心重设计：层级全景 + 跨层关联图谱（2026-07-25）

> 本节为记忆中心页面的重设计需求，落地后逐步替代 §5（记忆总览页）与 §10（知识图谱）描述的现行界面。设计细节见 `memory.design.md` 对应章节。

### 21.1 背景与问题

现行记忆中心（§5 描述的总览页）存在三个核心体验问题：

1. **层级核心不凸显**：总览页五层卡片为静态说明卡，仅展示层级名称与简介，用户无法看出「每层各记了多少、今天新增了多少、是否健康」。
2. **数据不直观**：用户无法感知记忆的流动（对话如何沉淀为知识、知识如何在聊天时被召回），也缺少可读的记忆动态事件流。
3. **缺少跨层关联可视化**：L4 实体图仅展示实体间关系，无法看到「实体 ↔ 事实 ↔ 情景」的跨层关联，记忆冲突（INHIBIT 边、事实冲突）在界面上不可见；L2 情景记忆在前端没有任何浏览入口。

### 21.2 用户故事

| 编号 | 用户故事 |
|------|---------|
| US-R1 | 作为用户，我希望打开记忆中心 3 秒内看懂「它记了多少、记了什么、记忆健不健康」，无需理解 L0-L4 术语。 |
| US-R2 | 作为用户，我希望看到记忆在五层之间如何流动（沉淀与召回），理解五层不是孤立列表而是一条管道。 |
| US-R3 | 作为用户，我希望用一张图看到人/事/概念（实体）与知识（事实）、经历（情景）之间的关联，并能从任一节点下钻。 |
| US-R4 | 作为用户，我希望记忆冲突（相互矛盾的事实）在图上直接可见，并能点击跳转处理。 |
| US-R5 | 作为用户，我希望看到「最近记忆动态」（什么时候提炼了什么知识、什么时候关联了新实体），让抽象的记忆写入变成可读事件。 |
| US-R6 | 作为用户，我希望浏览 Agent 经历过的情景片段（L2），并能从事实溯源到产生它的情景。 |

### 21.3 功能需求

| 编号 | 需求 | 优先级 |
|------|------|--------|
| FR-R1 | **层级全景首页**：五层卡片横排，每层展示一句话人话说明、核心大数字（记忆量）、今日新增、召回命中、健康徽标；层间箭头标注沉淀流量，双向流向条区分「沉淀 ⬇ / 召回 ⬆」 | P1 |
| FR-R2 | **需要关注区**：聚合展示知识冲突、待审批演进提案、上下文使用风险等待办，点击可跳转处理 | P1 |
| FR-R3 | **最近记忆动态**：跨层记忆事件流（提炼事实、关联实体、归档情景、召回命中），按时间倒序 | P1 |
| FR-R4 | **层级卡钻取**：点击层级卡跳转「记忆浏览」Tab 并过滤到该层 | P1 |
| FR-R5 | **跨层关联图谱 Tab**：L4 实体、L3 事实、L2 情景三类节点同图展示；节点形状+色码区分层级（圆形=实体、方块=事实、三角=情景），与首页色码全站统一 | P2 |
| FR-R6 | **图谱边类型**：实体关系边、实体↔事实关联边、事实↔事实链接边（A-MEM Links）、事实→情景来源边；冲突/抑制边以红色虚线警示 | P2 |
| FR-R7 | **图谱防毛线球**：默认以焦点节点为中心 2 跳展开，提供权重阈值滑块与层级开关过滤；支持搜索定位节点 | P2 |
| FR-R8 | **节点详情抽屉**：选中节点展示摘要、置信度、连接列表，支持「在记忆浏览中打开」跳转 | P2 |
| FR-R9 | **记忆浏览 Tab 整合**：facts 表格 + 新增 L2 情景时间线，支持按层过滤与从图谱跳转定位 | P3 |
| FR-R10 | **治理 Tab 归并**：级联审批、演进提案、平台设置、Worker 状态合并为一个治理入口 | P3 |
| FR-R11 | **扩散激活回放**：将已验证的扩散激活能力以逐层点亮的动画形式在图谱上回放 | P4 |

### 21.4 验收标准

- [ ] 新用户打开记忆中心首页，无需解释即可说出五层各自记什么、哪层记忆最多、哪层有问题。
- [ ] 用户能在图谱上找到任一实体关联的事实，并继续溯源到产生该事实的情景片段。
- [ ] 存在冲突事实时，用户在首页「需要关注」和图谱红边上都能看到入口，且能跳转处理。
- [ ] 图谱默认视图节点数受限（焦点 + 2 跳 + 权重阈值），不出现不可读的毛线球。
- [ ] L2 情景记忆在前端可浏览（此前完全无入口）。
- [ ] 所有枚举值与层级名称按项目 i18n 规范中文化展示。

### 21.5 非功能需求

| 类别 | 要求 |
|------|------|
| 性能 | 图谱默认限节点数（焦点 + 2 跳）；阈值过滤弱边；概览聚合接口单次请求返回全部层级统计 |
