...
| 项 | 状态 | 证据 |
|----|------|------|
| Episode 写入 | ✅ | `internal/data/memory_shim_l2.go` + `internal/data/memory_episode_sync.go`（InsertEpisodeRow，支持 pending/consolidated 状态） |
| 多策略 Recall | ✅ | `internal/data/memory_shim_l2_recall.go`（keyword + vector + importance + session boost 融合） |
| Embedding 索引 | ✅ | `internal/data/memory_shim_l2.go` + `internal/data/memory_episode_sync.go`（UpsertEpisodeEmbedding） |
...
| ConsolidateEpisode Worker | ✅ 已删除 | 原 `internal/cronrunner/jobs/memory_l2_consolidate.go` 已删除（MEM-OPT 3B-3：consolidation_status 统一为 "consolidated"，Worker 空转已移除） |
| Episode pending 状态 | ✅ | `internal/data/memory_shim_l2.go`（ConsolidationStatus 字段） |
...
- `internal/data/memory_shim_l2.go` + `internal/data/memory_episode_sync.go` — Episode 写入 + Embedding
- `internal/data/memory_shim_l2_recall.go` — 多策略 Recall
...
- ~~`internal/cronrunner/jobs/memory_l2_consolidate.go`~~ — 已删除（consolidation_status 统一为 "consolidated"）
