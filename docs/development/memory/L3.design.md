...
CREATE TABLE IF NOT EXISTS memory_facts (
  ...
  -- 白名单（auto-write pipeline 门禁）：preference / profile / goal / constraint / decision / relationship
  -- admin/手动路径额外允许：fact（通用兜底）；遗留值 rule/experience/pattern 仅展示回退
...
**As-built（2026-08-24）**：聊天热路径在 **有 query embedding 且 vector store 可用时一律走 pgvector + FTS + trigram RRF**，不再因为 scope 事实数 ≤5000 就改走全表扫描。词法扫描只用于无向量通道；该路径投影 `NULL AS embedding_blob`，避免每轮把向量 blob 拉进内存。

```text
1. 校验 agent.l3_enabled
2. 解析 scopes（默认 [agent,user,team,workspace,global]，按 agent 设置过滤）
3. 若 q.QueryEmbedding 为空 → 调 ProviderService.Embed(query)（失败则降级词法）
4. 候选生成（三选一）：
   a. vector store + query embedding → pgvector top-N ∪ FTS（内容词 OR tsquery）∪ pg_trgm word_similarity RRF
   b. 否则若 scope 活跃事实 ≤ brute-force 阈值 → 词法全扫（上限=阈值，无 blob）+ FTS/trigram 补漏
   c. 否则 → 事件时间 recency 池 + FTS/trigram（最多 200）
5. 每 hit 计算 calibrated hybrid：
   keyword 0.25 + vector 0.30 + importance×decay 0.20 + recency 0.15 + quality 0.10
   decay 用 valid_from/created_at（世界状态年龄；preference/identity 常青不衰）
   recency 用 last_used_at/updated_at（刚确认/刚用过仍置顶）
6. 过滤：先按 hybrid 排序，effective minScore = max(配置值, top1×0.6)；配置 ≤0 关闭过滤。截断 top_k 后再 CE rerank，不二次过滤
7. 按 max_chars 二次裁剪
8. 异步：每 hit BumpUseStat(hit=true)
9. 返回（含命中字段、来源、置信度、scores 分解）
```

问句是 Agent 的默认查询形态：`tokenizeQuery` 去掉功能词后，FTS 对内容词做 OR（`Alice | color`），而不是 AND 全词。中文短查询走 pg_trgm `word_similarity`（`%>`），因为 `'simple'` tsvector 不分词 CJK。这是聊天检索能力，不是评测特判。

对话即时抽取（`<fact>`）写入后，`ImmediateFactWriter` 对同槽偏好/住址/姓名做 `ShouldSupersedeSameSlotFact` 覆盖，不必等 Sleep-time。不走 `FactWritePipeline`（白名单会丢掉 identity/instruction）。
...
POST   /api/v1/memory/l3/facts/{id}/review {"action":"confirm|reject|archive|dispute|deprecate|refine", ...}
GET    /api/v1/memory/l3/facts/{id}/versions
GET    /api/v1/memory/l3/facts/{id}/feedback
POST   /api/v1/memory/l3/facts:bulk-upsert
```

> **ReviewMemoryFact**（2026-08-11 新增）：列定向 UPDATE，仅更新 `statement / details_markdown / fact_kind / tags_json / confidence / status`，不触碰 links / keywords / metadata / quality_score，防止治理操作误擦图谱关联。前端「记忆中心 → 知识库 → 操作列」confirm / reject / refine / archive 按钮均走此端点。手动创建事实默认 `confidence=0.85, source_kind='manual'`。
