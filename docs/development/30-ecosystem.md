...
| 组织架构整包（org_bundle） | 将公司组织架构整体打包发布：部门、岗位、岗位下挂的 Agent 一并携带，买家安装后一键生成完整组织与 Agent 阵容 |

> 组织架构整包是本模块的差异化需求：创作者可以把「一套公司建制」（如跨境电商客服公司：客服部 → 售前/售后岗位 → 各岗位 Agent）作为单个商品发布；买家侧在发布向导中通过组织节点树勾选打包范围，安装后按树还原部门/岗位/Agent。UI 骨架已覆盖该类型的浏览、详情预览（组织树）与发布向导。

### A3. 前端页面交互规格

商城采用六个页面组成的完整骨架（路由均为 `/shop` 前缀）：

| 页面 | 路由 | 用户可见内容 |
|------|------|------|
| 商城浏览首页 | `/shop` | Hero 搜索区（商城定位 + 搜索框 + 工作台/创作者中心/发布入口）、左侧分类树（研发/办公/客户/行业/内容等域，可多级展开）、过滤栏（资产类型 Chip 组 + 价格模型 + 排序 + 重置）、三榜单（热门安装/最新上架/高分好评）、资产卡片网格（类型图标、名称、创作者认证、价格标签、评分、安装量、一键安装） |
| 商品详情页 | `/shop/a/:slug` | 面包屑返回、资产头部（大图标 + 名称 + 类型 Chip + 版本 + 创作者 + 评分/安装量）、安装 CTA（免费直接装/付费购买安装/已安装可卸载）、权限确认弹窗（含高风险权限警示）、Tab 区（README Markdown 渲染 / 组织整包预览树 / 版本列表 / 评价列表与写评价）、右侧栏（权限清单、依赖、元信息、截图） |
| 创作者主页 | `/shop/u/:handle` | 创作者资料卡（头像、认证、简介、作品数/总安装/均分/关注者）、关注按钮、创作者全部作品网格 |
| 买家工作台 | `/shop/me` | 已安装 Tab（资产/版本/安装时间/7 日健康度/状态，支持更新与卸载）、订单 Tab（订单号/金额/下单时间/支付与退款状态） |
| 创作者中心 | `/shop/studio` | 统计卡片区（总安装/总收益/平均评分/在售资产，含趋势迷你图）、我的资产表格（安装量/评分/收益/更新时间/审核状态，支持新发版本与编辑）、评价收件箱 Tab（待回复评价、回复对话） |
| 发布向导 | `/shop/publish` | 步骤条发布流程：① 选择资产类型（11 类卡片，含 org_bundle）→ ② 基本信息（名称/slug 自动推导/分类/定价/README 编辑与预览/版本/兼容性/标签）→ ③ 组织整包专属步骤（组织节点树勾选部门/岗位/Agent，实时统计勾选数）→ ④ 确认发布（摘要表 + 组织整包勾选汇总） |

通用要求：

| 区域 | 用户可见内容 |
|------|------|
| 加载态 | 骨架屏（Skeleton）占位，避免布局跳动 |
| 空态 | 空列表展示引导插画与重置/去发布入口 |
| 通知反馈 | 安装/卸载/评价/加载失败均通过 Notify 提示，文案全部走 i18n |
| 响应式 | 桌面为左侧分类树右内容区；移动端折叠为单列 |
| 昼夜双模 | 亮色复用全局奶油白视觉，暗色使用 `body.body--dark` 深色玻璃拟态面板（`app-glass-panel`） |
