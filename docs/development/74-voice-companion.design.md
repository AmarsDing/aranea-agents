...
- **Prompt cache 前缀稳定化**（已落地 2026-08-11；2026-08-18 续修）：DeepSeek prompt caching 从 token 0 匹配，**全部** `role=system` 都算前缀。P1/P2 把动态 cue 挪到列表末尾后，真机仍出现 cached 锁在 ~6k（仅第一条静态 system）——尾部 system cue 仍被 provider 算进前缀。续修：动态 cue 改为 user 角色哨兵；静态 runtime cue 烘焙进 `WithInstruction`。详见 [28-callback.design.md §3.1](./28-callback.design.md#31-system-message-注入顺序前缀稳定化)
- **P2 深化：per-turn 动态 cue 末尾追加 + intent 搬移**（已落地 2026-08-11）：P1 后真机复测命中率仅 19%——`insertAfterLastSystem` 仍把 per-turn 动态 cue 插在 system 块后、history 前，history 每轮增长使可缓存前缀在动态 cue 处截断。P2 把动态 cue 改到列表末尾。2026-08-18 发现 DeepSeek 仍把末尾 `role=system` 算进前缀（cached 锁 ~6k），动态 cue 改为 user 角色哨兵，静态 cue 烘焙进 instruction。详见 [28-callback.design.md §3.1](./28-callback.design.md#31-system-message-注入顺序前缀稳定化)
