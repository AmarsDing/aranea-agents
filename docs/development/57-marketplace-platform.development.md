...
> **实现状态说明**：截至 2026-06-17，M57 模块 **尚未启动任何代码实现**。本计划引用的所有 M57 代码路径（`api/marketplace/v1/*.proto`、`cmd/marketplace/`、`internal/marketplace/*`、`internal/installer/`、`pkg/aranea-asset/`、`web/marketplace/`、`web/src/features/marketplace/`）均为 **规划中的未来代码锚点**，尚未创建。现有 M30 Ecosystem 模块是本计划的前置基础与现状对照。
...
| 层 | 路径 | 状态 |
|----|------|------|
| Proto | `api/marketplace/v1/{catalog,publish,install,social,payment,review}.proto` | 📋 待创建 |
| 商城后端 | `cmd/marketplace/` · `internal/marketplace/{biz,service,server,data}` | 📋 待创建 |
| Asset Schema | `pkg/aranea-asset/{manifest,pack,resolve,signer}/` | 📋 待创建 |
| 买家侧 Installer | `internal/installer/` · `internal/biz/installer/` · `internal/service/installer_service.go` | 📋 待创建 |
| 托管编排 | `internal/marketplace/orchestrator/` | 📋 待创建 |
| 公网 Web | `web/marketplace/` | 📋 待创建 |
| 主项目客户端 | `web/src/features/marketplace/` | 📋 待创建 |
| 配置 | `configs/marketplace.yaml` | 📋 待创建 |
| 红线 CI | `scripts/marketplace-boundary.sh` | 📋 待创建 |
...
| 2026-06-17 | 0.2 | 三件套内容边界整理：合并需求文档迁出的现状评估与里程碑；新增 M30 现有代码锚点与 M57 规划代码锚点对照；所有任务状态统一标记为 📋 待启动；修复跨文档引用链接（空格→短横线） |
