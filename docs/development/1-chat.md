...
## 子模块：任务执行总结报告（Execution Report）

> **状态**：2026-07-22 新增
> **设计**：详见 [1-chat.design.md §B.10.17](./1-chat.design.md)
> **开发计划**：详见 [1-chat.development.md §P-REPORT](./1-chat.development.md#p-report-任务执行总结报告execution-report2026-07-22)

### R.1 背景

编排类任务（多团队 DAG / 单团队 / 单执行单元）完成后，用户只能看到精灵的一段文字总结，缺少一份**结构化、可回看的执行报告**：任务是否成功、花了多久、消耗多少 token、每个执行单元产出了什么、有哪些交付物。现有 `SynthesisUsecase` 已生成综合结果并经 `synthesis_completed` 事件下发，但该事件为 WS-only 不落库，且前端卡片组件从未接入时间线（孤儿组件），刷新后无任何痕迹。

### R.2 用户故事

- 作为用户，当编排类任务执行完成（成功/部分失败/失败）时，我能在聊天时间线看到一张**任务执行总结报告卡片**，了解整体结果。
- 作为用户，报告卡片包含**执行概况**：原始需求、最终状态（已完成/部分失败/失败）、总耗时、token 消耗、执行单元成功率。
- 作为用户，报告卡片包含**分步结果明细**：每个团队/执行单元的名称、状态、耗时、产出摘要、失败原因。
- 作为用户，报告卡片包含**智能分析结论**：精灵基于全量上下文生成的核心发现、对比与建议。
- 作为用户，报告卡片包含**产物/交付物清单**：各执行单元沉淀的交付物摘要与大小。
- 作为用户，刷新页面或重新进入会话后，报告卡片仍在原位置完整显示（持久化回看）。
- 作为用户，当我主动中断任务时，**不**弹出总结报告（我已经知道任务被我停了）。
- 作为用户，简单问答（未启动规划编排）不产生报告卡片，避免打扰。

### R.3 功能需求

| # | 需求 | 优先级 |
|---|------|--------|
| FR-1 | 编排类任务终态（completed / partial_failure / failed）自动生成并下发执行报告 | P0 |
| FR-2 | 报告四板块：执行概况 / 分步结果明细 / 智能分析结论 / 产物交付物清单 | P0 |
| FR-3 | 报告以持久化事件落库，刷新/重连后可从历史还原 | P0 |
| FR-4 | 用户主动中断（cancelled）不生成报告 | P0 |
| FR-5 | 简单问答（PrePlanningGate 判定无需规划，直接回答）不生成报告 | P0 |
| FR-6 | LLM 结论生成失败时，报告降级为「无结论版」（结构化板块保留，结论区显示生成失败提示），不整个丢失 | P1 |
| FR-7 | 报告卡片状态色语义：已完成=绿、部分失败=黄、失败=红；多语言（zh-CN/en-US） | P0 |
| FR-8 | 报告在时间线中的位置附着于触发该编排的用户任务（Task），不渲染为用户输入气泡 | P0 |

### R.4 非功能需求

| 项 | 要求 |
|----|------|
| 可靠性 | 报告生成失败不得影响主流程（任务本身的状态推进与最终回复） |
| 防重 | 同一 spirit session 的一次编排只产生一张报告卡片（并发触发路径去重） |
| 性能 | 报告生成不阻塞终态事件发布；LLM 结论调用沿用现有 90s 超时 |

### R.5 验收标准

- [ ] 多团队任务全部完成后，时间线出现报告卡片，含四板块，数据与实际执行一致
- [ ] 单团队/单执行单元任务完成后同样出现报告卡片
- [ ] 部分失败任务：报告卡片状态为「部分失败」，失败单元显示错误原因，结论区给出分析
- [ ] 失败任务：报告卡片状态为「失败」
- [ ] 用户主动中断任务后，时间线**无**报告卡片
- [ ] 简单问答后，时间线**无**报告卡片
- [ ] 刷新页面后，报告卡片在原位置完整还原（内容与刷新前一致）
- [ ] 同一任务不产生重复报告卡片
- [ ] LLM 不可用时，报告卡片仍出现，结论区显示降级提示，其余板块完整
