...
| **核心导出** | `Registry()`、`Assemble()`、`AssemblyConfig`、`ToolRegistration`、`NormalizeInvocation`、`BatchExecuteAssembledTools`、`builtin_tools_seed.go` |
...
- Memory Admin RPC 须经 `authorizeMemoryScope`（`internal/service/memory_scope.go`）做 scope/workspace ACL；全景/图谱/情景及 Debug/CompositeSearch 另经 `assertAgentMemoryAccess`（Agent 租户 IDOR，跨租户 NotFound）；trpc `Add/Update/Delete/Clear` 须经 `assertL3WriteAllowed` 尊重 `WriteL3Facts`
...
| **核心导出** | `Ingest()`、`Retriever`、`Chunker`、`ExtractorRegistry`、`NewMemoryReranker`（`biz.Reranker`，AH-04 从 data 上移）；SP1：`blockparse.Parse()`（`internal/knowledge/blockparse/`，goldmark AST + wikilink 扫描纯函数）、`biz/knowledge.LinkIndex`（进程内链接内存图，五索引 + 版本号 + GraphDelta）、`Usecase.PromoteBlocks/PromoteDocuments/RebuildCollectionBlockIndex`；V4：`Usecase.ReembedDocuments/EnableCollectionSemantic`；**2026-08-15**：`GraphExpander` + Retrieve-Then-Generate + `AutolinkWikiMentions` + `WriteBackSessionFacts`（SP7 G2）+ `BackfillOutgoingAutolinks`（显式 POST autolink-backfill，**不**挂 RebuildKnowledgeIndex）+ 确认成链 custom HTTP + `LookupWriteBackHome` / GET writeback-home + `AgentMemoryProjector`（G1，覆盖写 `agents/{id}.md`）+ `CollectionHealthSnapshot` / `ListCollectionExperts`（G8/G7，专家打写回落点）+ pending 写回过门（逐条 `fact_ids`）；**2026-08-17**：RRF 并行召回与融合后重排、短中文词法强召回、查询 embedding 有界缓存、数据库上传图谱触发、relation evidence 原文门、`knowledge_links` 历史关闭保留、`KnowledgeIndexRepairWorker` 5 分钟派生索引自愈、标准 IR 指标与 30-query BM25 门禁（含自然语言问句内容针）、问句自动 RRF 路由、300 条改写鲁棒性金标、fact lineage 保留、事实 conflict 去重及 `keep_old`/`keep_new` 显式处置、`KnowledgeCurateWorker` 6h 低风险实跑、vault 同步 typed 关系钩子、`RebuildBlockIndex` 索引期成链不改源、`grounded_only` 拒答 cue、团队库 `EnsureEntityWikiPages`（写入即 replay chunks）、文档 `visibility`/`owner_user_id` ACL（含图一跳扩召回） |
...
| **核心导出** | `Runner`（含 Cancel）、`FrameworkBridge`、`PublishGate`、`ScoreDropAlerter`、UserSimulator（scripted/LLM） |
...
- **Orchestrate 死路径已删除（ADR-2，2026-08-20）**：`TaskOrchestratorPort.Orchestrate` 及 `orchestrateDAG/orchestrateTeam/orchestrateParallelTeams/orchestrateSingleAgent/sortByPerformance` 等 team 组装 helper 已整体移除；`TaskOrchestratorImpl` 仅保留 handle 记账（CheckProgress/Cancel/Synthesize/Recover/RecoverAllInterrupted）与在线学习回路。生产建团唯一路径：`PlanExecutor` + `RealTeamOrchestrator`（严格 DAG 调度），禁止回退旧编排器建团
...
| **测试覆盖** | Phase 0–2 + 花名册闭集已落地；Phase 4 分档/挂岗/点名剧本/前缀预算/合成锁/三管道/`graph_template_id`/确认五档（含 ConfirmBlock）/工具面/`collection_ids`/checkpoint 组织字段已测 |

**⚠️ 开发注意**：
- **禁止**任务热路径「LLM 找公司 → LLM 找部门 → 主管再分解 → 无则 CreateCompany」。公司 = 当前 workspace 树；创造公司/部门走 M67 管理面
- **禁止**把 `dept_lead` 选为业务 AssignedKey / 团队 Lead（除非 `plan_and_execute.agent_keys` 显式指定）
- 生产建团唯一路径仍是 `PlanExecutor` + `RealTeamOrchestrator`；本模块只补剪枝与 `DepartmentID` 断点
