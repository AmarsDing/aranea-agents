...
| 项 | 内容 |
|----|------|
| 入口 | 工具栏「清空」按钮（`delete_sweep`，无记录时禁用） |
| 确认 | 持久对话框显示当前总条数，需显式确认；文案声明不可撤销 |
| 行为 | 硬删除清空全部审计记录，返回删除条数并提示 |
| 可追溯 | 清空后自动写入一条 `delete.audit_logs`（warning）自审计条目，记录操作本身 |
...
| **筛选** | 关键字搜索（支持 agent/团队显示名与会话标题，2026-08-05 起）；状态筛选 chips、运行域筛选 chips（对话/团队/系统/技能，带行数计数）；**默认只显示「对话 + 团队」**，内部噪音（system/skill）默认排除、可经域 chips 显式查看；筛选变化自动回到第 1 页 |
| **实时** | 运行完成/失败时列表自动刷新；右上角 live pill 显示实时连接状态（绿「实时」/红「错误」/灰「连接中」） |
| **表格列** | Agent/Provider/Model、令牌 in/out（紧凑格式如 156.8k）、延迟、成本（$0.0220）、错误、相对时间（tooltip 为绝对时间）、详情操作；点击行打开详情 |
| **分页** | 服务端分页（每页 12 条 + 总条数） |
