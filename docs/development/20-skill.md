...
| `/skills/tags` | 标签字典 | 标签预建、改名合并、删除清理、孤儿标签收录 |

侧栏与 **`18 monitor.md`** 保持一致，使用父级 `QDrawer` + `QList` / `QItem`。路由入口展示在管理类导航下，文案分别为「Skill 管理」「运行记录」和「标签字典」。
...
> 磁盘同步相关字段（`filesystem_missing`、`sync_origin`、`SkillFilesystemHealth` 等）详见 [20-skill.design.md §五 API 契约](./20-skill.design.md#五api-契约)。
...
- **编写合同（验收）**：简介必须能和其他 Skill 区分（不得与名称同文）；`SKILL.md` 正文只写会改变决策的内容，超过 12 KiB 或 500 行时导入展示警告（不阻断）；长清单与脚本放 `references/` / `scripts/`。
...
| 标签 `tags` | `QSelect use-input use-chips` | 可为空；新增标签默认 `source=user`；选项源来自标签字典（字典 + 使用中标签），允许输入新标签 |
...
> 数据模型字段（`Skill` / `SkillVersionSummary` / `SkillTag` / `SkillInvocation`）、API 契约（REST 端点、请求/响应结构、错误格式）详见 [20-skill.design.md §四 数据模型](./20-skill.design.md#四数据模型) 与 [§五 API 契约](./20-skill.design.md#五api-契约)。

---

## 7. 运行时行为与前端组件

> 运行时行为（挂载条件、Agent 策略、Prompt 注入方式、Embedding 精排等实现结论）详见 [20-skill.design.md §七 运行时层](./20-skill.design.md#七运行时层) 与 [§十 Web 前端设计](./20-skill.design.md#十web-前端设计)（含 Quasar 组件清单）。
...
- 进入 `/skills/tags` 能看到按维度分组（`domain:` / `file_type:` 等前缀）的标签列表，无维度前缀的归入「通用标签」。
- 每个标签展示规范名称、收录状态（系统 / 已收录 / 未收录孤儿）和实时使用次数（聚合所有 Skill 的 `metadata_json.tags`）。
- 能新建预建标签；名称自动规范化为小写，非法格式（大写开头、空格、特殊字符）被阻止并提示。
- 改名后所有引用该标签的 Skill 在同一事务内同步重写，通知中展示重写条数；目标名已存在时等价于合并。
- 删除前确认文案区分「未使用」与「正被 N 个 Skill 使用」两种场景；删除后标签从所有 Skill 引用中移除。
- 孤儿标签（使用中但未收录）有醒目徽标与提示，支持一键「收录」进字典。
- 搜索框按名称 / 维度过滤，收录状态筛选支持「仅已收录 / 仅未收录 / 仅系统」。
- Skill 编辑对话框、Skill 列表筛选栏、Agent 设置「要求的标签（AND）」三处的标签选项均来自字典选项源。

---

## 子模块：标签字典管理

> 设计细节（数据模型 / API 契约 / 事务重写 / 缓存失效）见 [20-skill.design.md §4.5、§5.12、§6.11](./20-skill.design.md#45-skill_tags-标签字典表)；实现任务与改动文件见 [20-skill.development.md](./20-skill.development.md)。

### 用户故事

- 作为**管理员**，我希望标签有统一字典，避免 `Sales` / `sales` / `domain:sales` 等冗余写法散落各 Skill，导致筛选和路由效果不一致。
- 作为**管理员**，我希望能把两个同义标签合并（改名到同一目标），且所有引用自动重写，不需要逐个 Skill 手工修改。
- 作为**管理员**，我希望看到「使用中但未收录」的孤儿标签，一键收录或清理，让字典逐步收敛。
- 作为**普通用户**，我在编辑 Skill 标签或按标签筛选时，希望下拉选项来自字典，而不是凭记忆输入。

### 功能需求

| # | 需求 | 说明 |
|---|------|------|
| D1 | 标签预建 | 管理员可提前创建规范标签（含维度前缀），供各 Skill 选用 |
| D2 | 使用计数 | 列表实时聚合所有 Skill 的标签使用次数，不落库、保证强一致 |
| D3 | 改名合并 | 改名在同一事务内重写所有 Skill 的 `metadata_json.tags`；目标已存在时删除源行等价合并 |
| D4 | 删除清理 | 删除字典标签并从事务内所有 Skill 引用中移除，返回重写条数 |
| D5 | 孤儿治理 | 使用中但未收录的标签以 `orphan` 标记展示，支持收录（以同名预建）或删除清理 |
| D6 | 规范化校验 | 标签名统一小写、trim，必须匹配 `[a-z0-9][a-z0-9_-]*(:suffix)?`，维度取自 `:` 前缀 |
| D7 | 选项源复用 | Skill 编辑、列表筛选、Agent 设置三处标签下拉共用字典选项源，仍允许输入新标签 |
| D8 | 缓存一致性 | 改名 / 删除后失效 embed 路由缓存与去重缓存（`skillCorpusText` 含 tags，向量必须重算） |

### 交互规格

- 页面按维度分组展示：组头图标 + 维度名 + 组内数量 chip；「通用标签」组收纳无前缀标签。
- 行内操作：收录（仅孤儿行，`library_add` 图标）、改名（`edit`）、删除（`delete`），均带 `QTooltip` 说明。
- 改名对话框描述文案明确告知「所有引用该标签的 Skill 会在同一事务内同步重写」。
- 删除确认按使用计数分支文案（未使用 / 正被 N 个 Skill 使用）。
- 孤儿行使用浅黄底色（`warning` 6% 透明度）与普通行区分。
- 工具栏右侧汇总 chip：总标签数；有孤儿时追加 warning 色「N 个未收录」。

---

## 10. 运行时行为与前端组件

> 运行时行为（挂载条件、Agent 策略、Prompt 注入方式 C、Embedding 精排、Preview 选中原因等实现结论）与 Quasar 组件清单详见 [20-skill.design.md §七 运行时层](./20-skill.design.md#七运行时层) 与 [§十 Web 前端设计](./20-skill.design.md#十web-前端设计)。

---

*文档版本：4.1 — 新增标签字典子模块（`/skills/tags`）：需求、交互规格与验收标准；标签字段选项源接字典（2026-07-28）。*
