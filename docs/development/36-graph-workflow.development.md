...
| # | 差距 | 优先级 | 状态 | 说明 |
|---|------|--------|------|------|
| G1 | LLM 节点接线 | P1 | ✅ | `node_wiring.go` → `sg.AddLLMNode` |
...
| G9 | RetryPolicy / CachePolicy | P2 | ✅ | `node_wiring.go` → `WithRetryPolicy/WithNodeCachePolicy`，前端属性面板支持 |
| G10 | 用户自定义模板 | P2 | ✅ | `SaveGraphAsTemplate` + `metadata.user_template` + `GraphTemplatePicker` |
| G11 | Graph 版本管理 | P2 | ✅ | `metadata._version_history` + `ListGraphVersions/RollbackGraphVersion` + `GraphVersionPanel` |
| G12 | 导入/导出 | P2 | ✅ | `ExportGraph/ImportGraph` + 编辑器 ⋮ 菜单 + `useGraphEditorAssets.ts` |
| G13 | Webhook 通知 | P2 | ✅ | `GraphTaskRuntime.dispatchGraphTaskWebhook` → `graph.task.status` |
| G14 | 熔断策略 | P2 | 📋 | Proto `CircuitBreakerPolicy` 已定义但未接入 NodeDef；biz 层 `tool/circuit_breaker.go` 已实现 Tool 级熔断 |
| G15 | 节点布局持久化 | P1 | ✅ | `metadata.layout` + `graphLayout.ts` + dagre 自动布局 |
| G16 | Checkpoint/TimeTravel UI | P1 | ✅ | `GraphCheckpointPanel` + `useGraphTimeTravel` |
| G17 | Task 看板 UI | P1 | ✅ | `GraphTaskKanban` + `GraphTaskDetailDrawer` + WS 投影 |
...
- 熔断策略（G14）— Proto `CircuitBreakerPolicy` 已定义，需接入 NodeDef
- 子图嵌套编辑器 UI（G23）— 后端已支持，前端待补
...
- [x] LLM 节点可在画布上创建，执行时正确调用 LLM
- [x] Tool 节点可在画布上创建，执行时正确调用工具
- [x] Agent 节点 InputMapper/OutputMapper 正确映射状态
- [x] Command.GoTo 动态路由事件通过 WS 推送
- [x] ExecutionSummary 在执行完成后推送
- [x] 前端节点实时显示执行状态
- [x] 前端模板选择面板可用
- [x] 前端校验结果面板可用
...
- [x] 节点重试策略配置后自动重试
- [x] 用户可将 Graph 保存为自定义模板
- [x] Graph 版本管理支持查看历史和回滚
- [x] Graph 定义可导入导出
- [x] Webhook 通知正确触发
