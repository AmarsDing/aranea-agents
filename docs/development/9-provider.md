...
> - `PROVIDER_TYPE_OPTIONS` 已使用 trpc 枚举值
> - `VARIANT_OPTIONS` 已添加
> - 20 个 Provider 预设已对齐
...
| **出口** | 厂商请求/响应转换、SSE 流式归一、结构化日志与指标 | `adkruntime`/model_bridge + Provider 适配器；结构化日志与 span |
...
| 阶段 | 内容 | 前置依赖 |
|------|------|----------|
| 当前 | Failover/Hedge 模式已可用；Provider 适配器已实现 | — |
