...
> **版本**：2026-06-06 | **状态**：核心功能已实现，增强功能待开发
...
**代码锚点**：
- `api/kratos/avatar/v1/` — Avatar Proto 定义
- `internal/biz/avatar/` — Avatar Usecase + 图片处理 + 渠道图标刷新
...
- `web/src/stores/avatar/` — Pinia Store
...
| 图片自动中心裁剪 + 压缩 | ✅ | 前端 `prepareAvatarUpload` + 后端 `ProcessAvatarUpload`（512px 主图 + 128px 缩略图） |
...
| 8 | Wire 注入更新 | P2 | ❌ | `cmd/admin/wire.go` |
