...
### 3.6 空态与低数据量区块的空间控制（评审沉淀）

> 来源：2026-08-23 经验报告页 UI 评审——「根因分析」无数据卡片与「失败标签分布」单类目图表并排，两个半空卡片占住首屏约 1/3 高度，把报告列表挤出首屏。

| 场景 | 禁止 | 正确做法 |
|------|------|----------|
| 统计/图表卡片无数据 | 渲染「暂无数据」空卡片占住原版面（被 row 布局拉伸到与邻卡等高，半宽只放一句空文案） | 整块 `v-if` 不渲染；同排仅部分卡片有数据时，只渲染有数据的，空卡片不陪跑 |
| 图表类目极少（如柱状图仅 1 条） | 图表容器写死大高度（如 `height: 260px`），一根柱子悬在大片空白中 | 高度随类目数收缩（如 `min(48 + n × 28, maxHeight)`），或退化为紧凑文本/徽章统计 |
| 首屏空间分配 | 低信息量区块（空态、单条统计）占据首屏黄金位置，把列表/表格等高价值内容挤出首屏 | 高价值列表优先上首屏；低信息量区块收缩、降级或后置 |
| 同排卡片（row > col） | 卡片高度随内容各自为政，底边参差（列被 flex stretch 等高 ≠ 卡片等高） | 卡片根元素加 `full-height`（或 `height: 100%`）撑满列高，同排底边强制齐平 |
