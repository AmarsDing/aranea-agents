...
> **⚠️ 本文档自 2026-05-17 起停止维护**。状态表已被 [`docs/guides/execution-plan.md`](execution-plan.md) 附录 A "模块状态矩阵" 取代；本文 M1–M20 标注、§3 代码审计发现 与实际代码现状已脱钩（多模块的"已完成"实为代码骨架但未端到端接入主链路）。仅保留作历史参考。
>
> ---
>
> 本文档是项目对标 `pkg/trpc-agent-go` 框架的总体大纲，目标是 **完全复刻** 框架能力并 **超越**。
> 每个模块的详细需求见 `docs/需求/` 下对应文档。
> **最近一次代码审计**：2026-05-15（已过期，请以 `execution-plan.md` §1 为新基线）。
