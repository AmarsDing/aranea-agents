...
- **浮层视觉**（非纯逻辑）：须遵守 **[UX.md](../UX.md) §1～§2**：`background: var(--glass-elevated)`（或 `--glass-surface`）+ **`backdrop-filter` 与 `-webkit-backdrop-filter`** 成对；主按钮用 **`var(--color-accent)` / `--color-accent-hover`**，**禁止**在日间用夜间霓虹青紫作默认强调（UX §1）。
