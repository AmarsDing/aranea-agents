...
| 9 | 用量 / 监控 | **`usage/v1`**：**读** **`/v1/usage/*`**；**写**：**`POST /v1/usage/token-events`**（TokenUsageEvent JSON，**snake_case**；浏览器 ingest 与遗留 chat **勿双写**，见 [`pkg-backend-to-kratos.md`](./pkg-backend-to-kratos.md) **§6.3.2**）；**`monitor/v1`**（读）；SSE 仍部分遗留 | **`features/usage`、`features/monitor`**（`api.ts`：`createUsageService` / `createMonitorService`）；**`components/monitor`**（监控页与各 Tab 展示）；Trace 用量事件走 **`usage/v1`** | **`usage/v1`（读 + token-events 写）与 `monitor/v1`（读）已落地**；**SSE / 部分监控写入仍 pkg/backend** | **monitor 读路径已接 Kratos** | **待做** |

**维护约定**：每合并一域，将上表 **后端 / 前端 B / UX C** 更新为 **已落地**（或 **进行中**），并在 [`pkg-backend-to-kratos.md`](./pkg-backend-to-kratos.md) §6.3（**§6.3.1**：**`LEGACY_REST_ORIGIN` / Cron**；**§6.3.2**：用量 **浏览器 ingest / 语义双写**）保持同步。
