...
| 后端实现质量 | 16 | 20 | CRUD + 上传 + Session 筛选 ✅；`SignDownloadUrl` RPC + HTTP 签名下载 ✅ |
| 前端实现质量 | 11 | 15 | `ArtifactsPage.vue` ✅；Chat 侧栏 signed URL 打开 ✅；预览局限于文本 |
...
| 文本/代码预览 | ✅ |
| 签名下载 URL | ✅ |
| Chat 内附件引用（消息内嵌）| 🟡 侧栏 signed URL；消息 part 待补 |
| 跨会话制品检索 | ❌ |
...
| ID | 问题 | 建议修复 |
|----|------|---------|
| ART-P2-01 | 签名下载 URL | ✅ `SignDownloadUrl` + Chat 侧栏打开 |
| ART-P2-02 | Chat 内附件引用 | 🟡 消息 part 类型待规划 |
