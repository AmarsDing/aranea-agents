...
| agent 配置 model | `deepseek-chat` | **不在 provider_models 目录中** |
...
框架能力已存在：[`pkg/trpc-agent-go/model/openai/openai.go:560`](../../pkg/trpc-agent-go/model/openai/openai.go) `applyTokenTailoring` 在每次 `GenerateContent` 前按 `contextWindow`（catalog `context_window_k` 经 `WithContextWindow` 注入）+ 安全边距裁剪 messages（默认 MiddleOut 策略）。**但仅在模型 config_json 显式 `enable_token_tailoring=true` 时启用**。
