...
| 待执行队列 | 🟡 | 单 Agent：`pendingQueue` / `pendingCancels` + `processPendingQueue`；Team 会话串行保护待补；**Team turn 完成后不触发 processPendingQueue（Bug）** |
...
| SessionTurn 持久化 | 🟡 | 单 Agent：`recordSessionTurn` 写入 `session_turns`；Team turn 不调用 |
| Team Run 持久化 | ✅ | `CreateTeamRun` / `CreateTeamRunStep` 写入 `team_runs` / `team_run_steps` |
| EnvelopeError.PendingID | 🟡 | 字段已定义但未赋值；`processPendingQueue` 只写 metadata（双写问题） |
| WS 控制消息 | ✅ | `connected`/`pong`/`replay_start`/`replay_end`/`server_shutdown` 已实现但未文档化 |
| EventBuffer | ✅ | ring buffer 容量 200，支持 `last_event_id` 回放；无 TTL 清理 |
...
3. **P1**：**Team turn 完成后不触发 `processPendingQueue`**——Team 运行期间入队的消息永远不会被执行，这是功能性 Bug。
4. **P1**：AwaitUserReply 后端 API 已有，但 `AwaitHook` 仅注入单 Agent builder，Team 路径和前端 Chat 页回复 UI 未闭环。
5. **P1**：`EnvelopeError.PendingID` 字段已定义但未被赋值，`processPendingQueue` 只写 metadata；需统一到 `error.pending_id` 并让前端消费。
...
9. **P2**：Channel/Cron 并发入口可能绕过 `activeRuns` 互斥保护；Channel webhook 为并发入口，多个 webhook 可能同时触发同一 Session 的对话。
10. **P2**：Team turn 路径不调用 `recordSessionTurn`，导致 `session_turns` 表对 Team 会话无记录。
11. **P2**：EventBuffer 无 TTL 自动过期清理策略，长时间运行的 Session 可能积累大量事件。
...
| 4 | Team 路径注入 AwaitHook，Chat 页接入人工回复 UI | P1 | — |
...
- [ ] **Team turn 完成后，待执行队列中的消息能被正确消费**（processPendingQueue 触发）
- [ ] AwaitUserReply 在单 Agent 与 Team 路径均可触发，前端可提交回复
- [ ] `EnvelopeError.PendingID` 被正确赋值，metadata 双写已移除，前端可消费 `error.pending_id`
- [ ] Team 对话时前端可实时看到子 Agent 的增量输出
- [ ] 工具执行可展示结构化参数、结果、耗时、错误和 `is_long_running` 状态
- [ ] `session_turns` 表对 Agent 和 Team 会话均有记录
- [ ] Channel/Cron 并发入口不会绕过 activeRuns 互斥保护
...
- Phase 4 依赖 `EnvelopeError.PendingID` 统一后前端消费链路验证
- Phase 5 依赖 Reasoning 展示规格的产品决策
- Phase 7 依赖 LlmProvider 多模态能力（Vision API）与 Artifact/对象存储能力
- EventBuffer 当前无 TTL 清理，长时间运行 Session 可能内存泄漏
- Channel webhook 并发入口的 activeRuns 保护需要与 Channel 模块协调
