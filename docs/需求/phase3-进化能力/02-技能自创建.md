...
### 1.6 BabyAGI 启发：自构建工具框架

> 来源：BabyAGI functionz 框架（GitHub 22k+ stars），竞品分析差距 #8

BabyAGI 的 `functionz` 框架提供了"自构建自主智能体"的实验性实现，其核心思想——**Agent 能注册新函数、组合已有函数，扩展自身能力**——对本需求有直接启发：

| BabyAGI 能力 | 本需求对应 | 差异 |
|-------------|-----------|------|
| `process_user_input` 判断现有函数能否处理请求 | F1 重复模式检测 | BabyAGI 是即时判断，本需求是事后模式检测 |
| 自动生成新函数并注册到数据库 | F2 SKILL.md 生成器 + F5 Skill 注册 | BabyAGI 生成 Python 函数，本需求生成 SKILL.md |
| 函数依赖图追踪（imports/dependencies/key_dependencies） | 无对应 | **新增启发**：Skill 间依赖关系可借鉴 |
| Dashboard 可视化函数依赖图 | 无对应 | **新增启发**：前端 Skill 管理可增加依赖图视图 |
| Triggers 事件驱动自动执行 | F7 Skill 提议通知 | BabyAGI 是自动触发执行，本需求是通知 |

**可借鉴的增强方向**（对应 `docs/competitive-gap-requirements-2026-05-31.md` P2-8/P2-9）：

1. **工具缺口即时检测**：当前 F1 是事后模式检测（7 天 ≥5 次），可增加即时检测——Agent 运行时发现现有工具无法满足需求时，立即提议创建新 Skill（类似 BabyAGI 的 `process_user_input`）
2. **Skill 依赖声明**：在 SKILL.md 的 YAML front matter 中增加 `dependencies` 字段，声明 Skill 间的依赖关系，加载时自动解析
3. **Skill 依赖图可视化**：前端 Skill 管理页增加依赖图视图，类似 BabyAGI functionz Dashboard
