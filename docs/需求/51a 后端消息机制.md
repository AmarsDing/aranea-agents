...
原则 5：WebSocket 是主传输通道，Chat SSE 为兼容回退
...
| 7 | Monitor 日志与 Agent 事件割裂 | ✅ 已解决 | `SlogBridge` + `EnvelopeTypeLog` + WS channel:monitor |
...
| `MonitorLogBroker` + 独立端口 | 合并到 EventBus + SlogBridge，日志统一为 Envelope |
...
│  Chat SSE（兼容回退）       ← HTTP POST /v1/chat/messages/stream           │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
- **Envelope 同时服务于 WS 和 SSE**，传输层只负责编码方式
...
| `enqueue_message` | `handleEnqueueMessage` | 调用 ChatService.EnqueueUserMessage |
...
SlogBridge 将 Go 标准 `slog` 日志自动桥接到 EventBus：
...
| SlogBridge | 自动（函数名） | 桥接 Go slog 日志到 Envelope |
...
| `internal/event/slog_bridge.go` | SlogBridge：Go slog → EnvelopeTypeLog 桥接 |
...
| `internal/service/chat_native.go` | Chat SSE 为兼容回退，主路径走 EventBus → WS |
...
| `MonitorLogBroker` + 独立端口 | `EventBus` + SlogBridge + WS channel:monitor | ✅ 已删除 |
...
| `ChatService` HTTP 入口 | POST /v1/chat/messages/stream 不变（SSE 兼容回退） |
