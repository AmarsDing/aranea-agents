...
| **TTS（技术预览）** | [tts-development.md](./tts-development.md) | ⚠️ 占位模块；无生产 SLA |
...
| 优先级 | 项 |
|--------|-----|
| P2 | ~~设置页再瘦身~~ ✅ 页壳 ~251 行（样式外置）；Scanner TTL；LIST-04 列表 UI |
| P3 | 进化趋势图（AGT-16）、`GenerateAgentTitle`、迁移 |
...
## 技术预览与 P3 路线图（Review 判定）

> **不纳入核心 sprint**；UI 与文档已标 **技术预览**。单独立项后再投入实现。

| 模块 | 开发计划 | P3 下一步 |
|------|----------|-----------|
| CLI (25) | [25-cli-development.md](./25-cli-development.md) | 只读 `session list` / `chat send`；复用 admin auth |
| Ecosystem (30) | [30-ecosystem-development.md](./30-ecosystem-development.md) | 产品 schema、版本、审核流；非 mock 市场模型 |
| TTS | [tts-development.md](./tts-development.md) | Provider 选型；Chat 回复 → TTS → 播放；Channel 语音消息 |
| Skill import HTTP | [20-skill-development.md](./20-skill-development.md) | **设计例外**：`skill_import_http.go` 经 Service 鉴权；可选迁入 `SkillService` |
| Knowledge AgenticFilter | [37-knowledge-development.md](./37-knowledge-development.md) | Agent 决定是否检索；P3 backlog |
| GenerateAgentTitle | [8-agent-title-development.md](./8-agent-title-development.md) | LLM 自动标题 RPC；P3 |
