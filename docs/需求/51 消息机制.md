...
> - ✅ 7：Monitor 日志已统一为 `EnvelopeTypeLog`，通过 `SlogBridge` 自动桥接到 EventBus，走 WS `channel:monitor`。
...
| 5 | 无双向通信 | 用户中断、SteerableRunner.EnqueueUserMessage 无法通过现有连接上行 | ✅ 已解决：WS 上行 cancel / user_message / enqueue_message |
| 6 | 背压缺失 | 慢客户端导致 channel 满后事件被丢弃 | ✅ 已解决：三级 DropPolicy + Reliable 关键事件保障 |
| 7 | Monitor 日志与 Agent 事件割裂 | 运维日志走独立端口，与 Agent 事件无法统一订阅和过滤 | ✅ 已解决：`SlogBridge` + `EnvelopeTypeLog` + WS channel:monitor |
...
- 统一信封模型（Envelope）：项目层统一的事件传输单元，含 25 种事件类型
...
- 内部消费者：EventBusConsumer 统一处理 StateDelta 持久化、用量记录、EventBuffer 追加
- Monitor 日志统一接入：SlogBridge 自动桥接 slog → EnvelopeTypeLog → WS channel:monitor
...
**需求**：所有事件通过 WebSocket 统一传输，WS 为主通道，Chat SSE 为兼容回退
...
| 上行类型 | 说明 |
|---------|------|
| `user_message` | 发送聊天消息（content + agent_key + team_id + options） |
| `cancel` | 停止当前生成 |
...
| 消费者 | 订阅条件 | 职责 | 状态 |
|--------|---------|------|------|
| EventBusConsumer | 全量（Reliable） | 统一入口：EventBuffer 追加 + StateDelta 持久化 + Usage 记录 | ✅ 已实现 |
...
| `web/src/features/chat/envelope.ts` | Envelope 类型定义（25 种 EnvelopeType + WsDownstream + WsUpstream） |
