...
补充记忆系统的两大核心能力：**自动提取**（Agent 运行后自动从对话中提取重要信息写入 L2/L3）和 **检索增强**（L3 向量检索与 Agent 运行时注入），以及记忆管理界面（Memory Center）的完整前端设计。
...
type MemoryService struct {
    ...
    uc *biz.MemoryManagementUsecase
}

func NewMemoryService(uc *biz.MemoryManagementUsecase) *MemoryService
...
func (s *MemoryService) GetMemoryOverview(ctx context.Context, req *GetMemoryOverviewRequest) (*MemoryOverview, error)
...
待新增：
```
data.ProviderSet → NewMemoryExtractionRepo, NewMemoryConflictRepo
