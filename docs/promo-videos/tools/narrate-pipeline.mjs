...
function ffmpegConcat(inputs, output) {
  ...
  execSync(
    `"${process.env.FFMPEG_PATH || 'ffmpeg'}" -y -f concat -safe 0 -i "${listFile}" -c copy "${output}"`,
...
function makeSilence(duration, outPath) {
  execSync(
    `"${process.env.FFMPEG_PATH || 'ffmpeg'}" -y -f lavfi -i anullsrc=r=24000:cl=mono -t ${duration} -q:a 9 -acodec libmp3lame "${outPath}"`,
    { stdio: ['ignore', 'pipe', 'pipe'] },
  );
}
...
async function main() {
  ...
  for (let i = 0; i < scenes.length; i++) {
    ...
    for (let j = 0; j < chunks.length; j++) {
      ...
      // 断点续跑：chunk 已存在则复用，跳过 TTS（适用于上次中途失败的场景）
      let duration;
      if (fs.existsSync(chunkPath) && fs.statSync(chunkPath).size > 0) {
        duration = getDuration(chunkPath);
        console.error(`  chunk ${j}: 复用已存在 ${duration.toFixed(2)}s`);
      } else {
        const result = callTTS(chunk.text, chunkPath, { voice, speed });
        duration = result.duration;
        console.error(`  chunk ${j}: ${duration.toFixed(2)}s · ${chunk.text.length} 字 · ${chunk.text.slice(0, 30)}${chunk.text.length > 30 ? '…' : ''}`);
      }
      const chunkStart = sceneInternalCursor;
      chunkFiles.push(chunkPath);
      sceneInternalCursor += duration;
      chunkRecords.push({
        text: chunk.text,
        start: chunkStart,
