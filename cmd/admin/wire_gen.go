...
func wireApp(confServer *conf.Server, confData *conf.Data, runtime *conf.Runtime, debugRecorder *conf.DebugRecorder, logger log.Logger, loggatewayLogger loggateway.Logger, pipeline logpipeline.Pipeline, arg []*conf.LoggingSink) (wireOut, func(), error) {
	...
	runtimeTooling := provideRuntimeTooling(plugintrpcRuntime, manager, repository, retriever, adaptiveRouter, federatedRetriever, retrievalEvaluator, v27, factory, kanbanToolBridge, recorderFactory, organizationUsecase, toolResultGate, router, subagentService, parallelToolExecutor)
	...
	turnDeps := provideTeamTurnDeps(v25, agentRepository, agentUsecase, v2, v21, llmProviderModelUsecase, v10, systemSettingRepo, persistenceSet, sessionCompressor, v2Bus, monitorBus, sequencer, loggatewayLogger)
	...
	spiritTeamUsecase := provideSpiritTeamUsecase(teamUsecase, v25, agentUsecase, spiritTransactor, orchestrationCache, evolutionSuggestionRepo, verificationGateExecutor, deptLeadManager, stepV2Repo, sessionRuntime, loggatewayLogger)
	...
	chatOrchestratorDeps := provideChatServiceDeps(runRegistry, pendingMessageQueue, v29, v25, agentRepository, agentUsecase, v2, v21, llmProviderModelUsecase, v10, systemSettingRepo, persistenceSet, sessionRuntime, sessionCompressor, v2Bus, monitorBus, runtimeTooling, teamOrchestrationDeps, channelTurnJobDeps, channelNotifierDeps, v14, v15, mcpServerUsecase, v12, spiritTeamAssembler, spiritSynthesisService, orchestrationCache, teamStarter, graphService, taskOrchestratorPort, skillEvolutionUsecase, evolutionUsecase, skillInvocationStatsReader, router, subagentService, experienceAnalyticsUsecase, turnLifecycleUsecase, stepV2Repo, stepV2Repo, taskV2Repo, runHeartbeatEmitter, deadLetterQueue, profileResolver, projectorFactory, memberSessionV2Repo, memoryConsolidationWriter, loggatewayLogger)
	...
	v31 := cron.NewUsecase(cronRepo, cronrunnerRunner)
	cronService := service.NewCronService(v31)
	scopeAgentLookup := biz.NewScopeAgentLookup(agentRepository)
	v32 := plugin.NewUsecase(v18, v19, scopeAgentLookup)
	pluginService := service.NewPluginServiceWithBootstrap(v32, plugintrpcRuntime, loggatewayLogger)
	...
	v33 := service.ProvideSkillResolveRootFn(systemSettingRepo)
	v34 := storage.NewSkillFilesystem(v33)
	...
	skillService := service.NewSkillService(v10, agentUsecase, skillHealthUsecase, v34, engine, loggatewayLogger)
	...
	channelIngress := provideChannelIngress(channelUsecase, channelTurnJobUsecase, v25, chatService, graphService, cronTriggerGateway, v2Bus, turnAdmissionUsecase, teamCompiler, loggatewayLogger)
	...
	v35 := flowlog.NewUsecase(flowlogRepo)
	flowLogService := service.NewFlowLogService(v35)
	...
	v36 := provideDiagBundleGenerator(v5, v6, rootCauseEngine)
	v37 := provideSelfHealUsecase(v36, loggatewayLogger)
	v38 := data.NewHealRecordRepo(dataData)
	v39, err := provideSelfHealObserver(runtime, v38, rootCauseEngine, v9, loggatewayLogger)
	if err != nil {
		cleanup()
		return wireOut{}, nil, err
	}
	...
	v40 := monitor.ProvideSelfCheckers(dbPinger, traceProjector, alertEvalWorker, eventBusHealthChecker, wsConnectionCounter, flowFileAppender)
	eventBusResubscriber := provideEventBusResubscriber()
	v41 := monitor.ProvideSelfCheckRepairers(flowFileAppender, traceProjector, alertEvalWorker, eventBusResubscriber)
	selfCheckReportRepo := data.NewSelfCheckReportRepo(dataData)
	alertMetricRegistry := monitor.NewAlertMetricRegistry()
	selfCheckScheduler := provideSelfCheckScheduler(v40, v41, selfCheckReportRepo, alertMetricRegistry, loggatewayLogger)
	monitorService := service.NewMonitorService(v12, processLogEnabledProvider, flowLogService, codeExecutorService, v36, v37, v39, selfCheckScheduler, selfCheckReportRepo, loggatewayLogger)
	...
	serviceArtifactService := service.NewArtifactService(v15, signer, sessionWorkspaceLookup)
	...
	v42 := evaluation.NewUsecase(evaluationRepo, loggatewayLogger)
	evaluationRunner := service.ProvideEvaluationRunner(chatService, chatService, v42, llmProviderModelUsecase, systemSettingRepo, loggatewayLogger)
	evaluationService := service.NewEvaluationService(v42, evaluationRunner)
	...
	v43 := ecosystem.NewUsecase(ecosystemRepo, loggatewayLogger)
	ecosystemService := service.NewEcosystemService(v43)
	...
	eventBusSideConsumers := provideEventBusSideConsumers(infra, v2Bus, v21, webhookDispatcher, v25, v35, v12, turnMemoryWorker, traceProjector, flowFileAppender, v29, sessionLogWriter)
	...
	flowLogCleanup := provideFlowLogCleanup(v35, loggatewayLogger)
	monitorAlertCooldownCleanup := provideMonitorAlertCooldownCleanup(v12, logger)
	autoHealTTLCleanup := provideAutoHealTTLCleanup(v38, loggatewayLogger, logger)
	...
	predictiveHealUsecase := providePredictiveHealUsecase(v12, failurePatternReadWriter, v38, loggatewayLogger)
	predictiveHealJob := providePredictiveHealJob(predictiveHealUsecase, loggatewayLogger)
	patternMiningUsecase := providePatternMiningUsecase(v38, failurePatternReadWriter, failurePatternReadWriter, loggatewayLogger)
...
func provideChannelIngress(
	...
	return service.NewChannelIngress(channels, turnJobs, sessions, chat, graphs, cron2, eventBus, dedupe, debouncer, registry, gate, admission, teamCompiler, lg)
}
...
func provideSpiritTeamUsecase(teamUC *biz.TeamUsecase, sessionUC *biz.SessionUsecase, agentUC *biz.AgentUsecase, transactor biz.SpiritTransactor, orchCache *biz.OrchestrationCache, evolutionSugg biz.EvolutionSuggestionRepo, gateExecutor *biz.VerificationGateExecutor, deptLeadMgr *biz.DeptLeadManager, stepReader biz.StepV2Reader, sessionRT *session2.Runtime, lg loggateway.Logger) *biz.SpiritTeamUsecase {
	return biz.NewSpiritTeamUsecase(teamUC, sessionUC, agentUC, lg, biz.WithSpiritTransactor(transactor), biz.WithOrchestrationCache(orchCache), biz.WithEvolutionSuggestionRepo(evolutionSugg), biz.WithVerificationGateExecutor(gateExecutor), biz.WithDeptLeadMgr(deptLeadMgr), biz.WithSpiritStepReader(stepReader), biz.WithGraphDeliverableReader(service.NewGraphDeliverableReader(sessionRT)))
}
