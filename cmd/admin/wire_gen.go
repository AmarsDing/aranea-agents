...
func wireApp(confServer *conf.Server, confData *conf.Data, runtime *conf.Runtime, selfImprovement *conf.SelfImprovement, sandbox *conf.Sandbox, debugRecorder *conf.DebugRecorder, logger log.Logger, loggatewayLogger loggateway.Logger, pipeline logpipeline.Pipeline, arg []*conf.LoggingSink) (wireOut, func(), error) {
	...
	v11 := provideUsageUsecaseRef()
	multiProviderEmbedder := service.NewKnowledgeEmbedder(confData, systemSettingRepo, v11, loggatewayLogger)
	...
	v12 := data.NewSkillTagRepo(dataData)
	v13 := provideSkillUsecase(v10, multiProviderEmbedder, skillDedupUsecase, v12, loggatewayLogger)
	v14 := provideFilesystemHealthReader(v13)
	v15 := data.NewMonitorTraceSpanReader(dataData)
	...
	v16 := alert.NewAlertMetricRegistry()
	v17 := data.NewMonitorTraceUsageRepo(dataData)
	v18 := provideTraceProjector(v6, v17, infra, loggatewayLogger)
	v19 := provideFlowFileAppender(loggatewayLogger)
	v20 := provideMonitorUsecase(v4, v5, v6, v7, v8, v9, v14, v15, sequencer, memoryCanaryStatus, v16, v3, v18, v19, loggatewayLogger)
	v21 := data.NewA2ARepoFromData(dataData, loggatewayLogger)
	agentLookup := biz.ProvideA2AAgentLookup(agentRepository)
	v22 := a2a.NewUsecase(v21, v21, v21, v21, agentLookup)
	...
	v23 := artifact.NewUsecase(artifactRepo, loggatewayLogger)
	artifactService := provideArtifactRuntimeService(v23, loggatewayLogger)
	...
	memoryAdminDeps := provideMemoryAdminDeps(dataData)
	...
	repository := service.NewSkillDBRepository(v13, loggatewayLogger)
	evolutionService := provideEvolutionService(llmProviderModelUsecase, repository, v11, loggatewayLogger)
	persistenceSet := providePersistenceSet(dataData, agentMCPTooling, sessionService, artifactService, v23, memoryService, memoryJobQueue, memoryPolicyEngine, memoryL2Recaller, memoryL3Recaller, memoryCompositeRecaller, memoryAdminUsecase, l4Reconsolidator, loggatewayLogger, memoryJobDeadLetterRepo, evolutionService)
	...
	agentService := service.NewAgentService(agentUsecase, evolutionUsecase, v20, v22, promptFileAIEditor, agentTemplateUsecase, loggatewayLogger, monitorBus)
	llmProviderModelService := service.NewLlmProviderModelService(llmProviderModelUsecase, v20, loggatewayLogger)
	hookRepo := data.NewHookRepo(dataData)
	v24 := hook.NewUsecase(hookRepo, loggatewayLogger)
	deliveryRepo := data.NewHookDeliveryRepo(dataData)
	v25 := hook.NewDeliveryUsecase(deliveryRepo)
	v26 := data.NewPluginRepo(dataData)
	v27 := data.NewPluginRunRepo(dataData)
	statsRecorder := providePluginStatsRecorder(v26, v27, agentRepository, runtime, loggatewayLogger)
	v28 := data.NewPluginCostGuardUsageRepo(dataData)
	...
	v29 := provideToolUsecaseWithDeps(v2, settingRepo, toolTester, toolWebResearchReadinessChecker, toolGrantRepo, toolParamRuleRepo, loggatewayLogger)
	plugintrpcRuntime := providePluginRuntime(statsRecorder, v28, v29, deliveryRepo, monitorBus, loggatewayLogger)
	v30 := hook.NewResolver(v24, loggatewayLogger)
	manager := providePluginManager(plugintrpcRuntime, v30, agentRepository, loggatewayLogger)
	hookService := service.NewHookService(v24, v25, manager, loggatewayLogger)
	cronRepo := data.NewCronRepo(dataData)
	v31 := data.NewSessionMetricsRepo(dataData)
	v32 := data.NewSessionMetricsReader(dataData)
	sessionMetricsCache := data.NewSessionMetricsCache(v32, loggatewayLogger)
	sessionRepo := data.NewSessionRepo(dataData, v31, sessionMetricsCache)
	sessionAgentLookup := biz.NewSessionAgentLookup(agentRepository)
	teamLookup := biz.NewSessionTeamLookup(teamRepo)
	sessionTitleGenerator := provideSessionTitleGenerator(llmProviderModelUsecase, v11, persistenceSet, loggatewayLogger)
	...
	v33 := session.NewSessionUsecase(sessionRepo, sessionAgentLookup, teamLookup, sessionTitleGenerator, sessionParticipantRepository, sessionStatusPublisher, sessionMetricsUsecase, sessionRuntimeWriter, activityLister, loggatewayLogger, sessionFlowLogWriter)
	...
	sessionLeases := provideSandboxSessionLeases(sandboxManager)
	factory := provideCodeExecutorFactory(loggatewayLogger, sandboxManager, sessionLeases)
	v34 := data.NewKnowledgeRepoFromData(dataData)
	retriever := service.NewKnowledgeRetriever(multiProviderEmbedder, v34, loggatewayLogger)
	vaultFiler := provideKnowledgeVaultFiler(loggatewayLogger)
	blockIndexRepo := data.NewKnowledgeBlockRepoFromData(dataData)
	v35 := biz.ProvideKnowledgeUsecase(v34, vaultFiler, blockIndexRepo, loggatewayLogger)
	...
	subagentService, err := provideSubAgentService(v11, loggatewayLogger)
	if err != nil {
		cleanup()
		return wireOut{}, nil, err
	}
	...
	graphNodeResolverSet := provideGraphBuildDeps(llmProviderModelUsecase, v29, agentUsecase, agentRepository, systemSettingRepo, v13, persistenceSet, repository, factory, retriever, v35, manager, organizationUsecase, toolResultGate, router, subagentService, v22, nodeCircuitBreakerRegistry, bridge, runtime, lifecycle, loggatewayLogger)
	...
	v36 := data.NewSessionRuntimeReader(dataData)
	compiledTeamRepo := data.NewCompiledTeamRepo(dataData, v36)
	...
	memoryResync := provideSessionMemoryResync(memoryAdminDeps)
	...
	v38 := data.NewKnowledgeSparseSearcherFromData(dataData)
	hybridRetriever := service.NewKnowledgeHybridRetriever(retriever, v38, loggatewayLogger)
	...
	graphExpander := service.NewKnowledgeGraphExpander(v34, loggatewayLogger)
	adaptiveRouter := service.NewKnowledgeAdaptiveRouter(hybridRetriever, queryRewriter, graphExpander, v34, loggatewayLogger)
	federatedRetriever := service.NewKnowledgeFederatedRetriever(adaptiveRouter, retriever, v35, loggatewayLogger)
	...
	v39 := provideM71MessageReader(activityLister)
	globalMessageSearcher := data.NewGlobalMessageSearchRepo(dataData)
	sessionSearchUsecase := provideSessionSearchUsecase(agentRepository, sessionRepo, v39, globalMessageSearcher, accessAuditor, loggatewayLogger)
	...
	runtimeTooling := provideRuntimeTooling(plugintrpcRuntime, manager, repository, skillHealthMetricsAdapter, retriever, adaptiveRouter, federatedRetriever, retrievalEvaluator, v35, factory, kanbanToolBridge, recorderFactory, organizationUsecase, toolResultGate, router, subagentService, parallelToolExecutor, resourceAccessUsecase, deptMailboxUsecase, sessionSearchUsecase, bridge, computerUseUsecase, agentBridgeService, sessionLeases, runtime)
	...
	skillRegistrationPort := provideSkillRegistrationPort(v13)
	...
	memoryLLMExtractorConfig := provideMemoryLLMExtractorConfig(agentUsecase, v33, llmProviderModelUsecase, v11, loggatewayLogger)
	...
	teamRunHeartbeatWriter := data.NewTeamRunHeartbeatWriter(dataData, loggatewayLogger)
	runner := team.ProvideRunner(teamRepo, teamRepo, teamRepo, teamUsecase, teamRepo, teamRepo, v37, v20, turnDeps, repository, factory, loggatewayLogger, runnerConfig, teamRunHeartbeatWriter)
	...
	verificationGateExecutor := provideVerificationGateExecutor(deptLeadManager, dynamicLLMCaller, v13, deptMailboxUsecase, loggatewayLogger)
	spiritTeamRunStatsReader := data.NewSpiritTeamRunStatsReader(dataData, loggatewayLogger)
	spiritTeamRunHeartbeatReader := data.NewSpiritTeamRunHeartbeatReader(dataData, loggatewayLogger)
	spiritTeamUsecase := provideSpiritTeamUsecase(teamUsecase, v33, agentUsecase, spiritTransactor, orchestrationCache, evolutionUsecase, verificationGateExecutor, deptLeadManager, deptMailboxUsecase, stepV2Repo, spiritTeamRunStatsReader, spiritTeamRunHeartbeatReader, sessionRuntime, systemSettingRepo, artifactRepo, loggatewayLogger)
	...
	sessionRunEscalationNotifier := provideChannelRunEscalationNotifier(channelUsecase, v33, loggatewayLogger)
	channelNotifierDeps := provideChannelNotifierDeps(sessionRunEscalationNotifier)
	teamStarter := service.NewTeamStarter(v33, teamOrchestrationDeps, v2Bus, sequencer, loggatewayLogger, taskV2Repo, teamStageV2Repo, teamRunV2Repo)
	...
	skillIntelligenceUsecase := provideSkillIntelligenceUsecase(skillScoringUsecase, skillReportUsecase, unifiedEvolutionRepo, skillIntelligenceRepo, skillIntelligenceRepo, skillEvolutionOrchestrator, skillDraftEvolver, skillReloader, skillRegistrationPort, loggatewayLogger)
	...
	turnLifecycleUsecase := provideTurnLifecycleUsecase(v33, loggatewayLogger)
	...
	deps := provideCronRunnerDeps(cronRepo, v33, teamRepo, agentRepository, v2Bus, monitorBus, chatService, modelRegistrySyncAgent)
	cronrunnerRunner := provideCronRunner(deps, dataData, loggatewayLogger)
	v40 := cron.NewUsecase(cronRepo, cronrunnerRunner)
	cronService := service.NewCronService(v40)
	scopeAgentLookup := biz.NewScopeAgentLookup(agentRepository)
	v41 := plugin.NewUsecase(v26, v27, scopeAgentLookup)
	pluginService := service.NewPluginServiceWithBootstrap(v41, plugintrpcRuntime, loggatewayLogger, monitorBus)
	mcpServerService := service.NewMCPServerService(mcpServerUsecase, v20, agentUsecase, loggatewayLogger, monitorBus)
	...
	v42 := service.ProvideSkillResolveRootFn(systemSettingRepo)
	v43 := storage.NewSkillFilesystem(v42)
	...
	v44 := evaluation.NewUsecase(stores, loggatewayLogger)
	evaluationRunner := service.ProvideEvaluationRunner(chatService, chatService, v44, llmProviderModelUsecase, systemSettingRepo, agentUsecase, v2Bus, learningLoopUsecase, v37, loggatewayLogger)
	publishGate := service.ProvidePublishGate(v44, evaluationRunner, v2Bus, loggatewayLogger)
	...
	toolService := service.NewToolService(v29, agentUsecase, v20)
	v45 := data.NewSessionForkRepo(dataData)
	v46 := biz.ProvideSessionForkUsecase(sessionRepo, v45, loggatewayLogger)
	...
	serviceSessionService := service.NewSessionService(v33, v46, v20, sessionRunUsecase, sessionCompressor, sessionCompressor, v32, sessionV2Service, loggatewayLogger)
	cronTriggerGateway := service.NewCronTriggerGatewayAdapter(cronService)
	turnAdmissionUsecase := provideChannelIngressAdmission(v37, agentRepository, channelUsecase)
	channelGateCards := provideChannelGateCards(v2Bus, v33, channelUsecase, chatService, stepV2Repo, loggatewayLogger)
	channelIngress := provideChannelIngress(channelUsecase, channelTurnJobUsecase, v33, chatService, graphService, cronTriggerGateway, v2Bus, monitorBus, turnAdmissionUsecase, teamCompiler, channelGateCards, loggatewayLogger)
	...
	channelService := service.NewChannelService(channelUsecase, channelTurnJobUsecase, channelRuntime, v20, loggatewayLogger)
	usageService := service.NewUsageService(v37)
	processLogEnabledProvider := provideProcessLogEnabled(confServer)
	flowlogRepo := data.NewFlowLogRepo(dataData)
	v47 := flowlog.NewUsecase(flowlogRepo)
	flowLogService := service.NewFlowLogService(v47)
	codeExecutorService := service.NewCodeExecutorService(factory, loggatewayLogger)
	v48 := provideDiagBundleGenerator(v5, v6, rootCauseEngine)
	v49 := data.NewHealRecordRepo(dataData)
	v50, err := provideSelfHealObserver(runtime, v49, rootCauseEngine, v9, loggatewayLogger)
	if err != nil {
		cleanup()
		return wireOut{}, nil, err
	}
	dbPinger := provideDBPinger(dataData)
	v51 := provideMonitorAlertEvalWorker(v20)
	...
	v52 := monitor.ProvideSelfCheckers(dbPinger, v18, v51, eventBusHealthChecker, wsConnectionCounter, v19, v8)
	eventBusResubscriber := provideEventBusResubscriber()
	v53 := monitor.ProvideSelfCheckRepairers(v19, v18, v51, eventBusResubscriber)
	...
	selfCheckScheduler := provideSelfCheckScheduler(v52, v53, selfCheckReportRepo, v16, loggatewayLogger, monitorFlowLogWriter)
	monitorService := service.NewMonitorService(v20, processLogEnabledProvider, flowLogService, codeExecutorService, v48, v50, selfCheckScheduler, selfCheckReportRepo, loggatewayLogger)
	...
	serviceMemoryService := provideMemoryService(persistenceSet, l4CascadeUsecase, systemSettingUsecase, memoryJobDeadLetterRepo, memoryJobQueue, memoryJobQueue, memoryWorkerStats, dataData, agentUsecase, loggatewayLogger)
	...
	endpointRegistry := provideA2AEndpointRegistry(a2AEndpointBuilder, v22, publicBaseURLStore, loggatewayLogger)
	a2APublicBaseReloader := provideA2APublicBaseReloader(publicBaseURLStore, endpointRegistry, publicBaseURLInput)
	systemSettingService := service.NewSystemSettingService(systemSettingUsecase, a2APublicBaseReloader, credentialCrypto, multiProviderEmbedder, v20, loggatewayLogger, monitorBus)
	...
	teamService := service.NewTeamService(teamUsecase, graphUsecase, agentUsecase, v33, runner, runRegistryPort, v2Bus, loggatewayLogger, spiritSynthesisService, teamStageV2Repo, stepV2Repo, stepV2Repo, teamRunV2Repo, memberSessionV2Repo, sequencer, v20, v37, queryUsecase)
	signer := provideArtifactSigner(loggatewayLogger)
	sessionWorkspaceLookup := service.ProvideSessionWorkspaceLookup(v33)
	sessionWorkspaceSearcher := service.ProvideSessionWorkspaceSearcher(v33)
	serviceArtifactService := service.NewArtifactService(v23, signer, sessionWorkspaceLookup, sessionWorkspaceSearcher)
	...
	entityPipeline := provideKnowledgeEntityPipeline(dataData, dynamicLLMCaller, systemSettingUsecase, llmProviderModelUsecase, v35, loggatewayLogger)
	relationExtractor := provideKnowledgeRelationExtractor(dataData, dynamicLLMCaller, systemSettingUsecase, llmProviderModelUsecase, v35, loggatewayLogger)
	writeBackGraphFunc := provideKnowledgeWriteBackGraphHook(entityPipeline, relationExtractor, loggatewayLogger)
	knowledgeService := service.NewKnowledgeService(v35, multiProviderEmbedder, knowledgeSearchDeps, markdownOrganizer, extractorRegistry, assetStore, v2Bus, systemSettingRepo, writeBackGraphFunc, loggatewayLogger)
	evaluationService := service.NewEvaluationService(v44, evaluationRunner)
	redisClient := provideRedisClient(confData, loggatewayLogger)
	limiter := provideA2ALimiter(redisClient, loggatewayLogger)
	a2AService := provideA2AService(v22, chatService, agentRepository, endpointRegistry, publicBaseURLStore, limiter, loggatewayLogger)
	...
	federationUsecase := provideFederationUsecase(a2AFederationRepo, federationGovernance, v21, v21, remoteAgentCardWriter, remoteInvokeExecutor, loggatewayLogger, flowLogWriter)
	federationService := provideFederationService(federationUsecase)
	ecosystemRepo := data.NewEcosystemRepo(dataData)
	v54 := ecosystem.NewUsecase(ecosystemRepo, loggatewayLogger)
	ecosystemService := service.NewEcosystemService(v54)
	...
	skillReplayRunner := provideSkillReplayRunner(v44, dynamicLLMCaller, systemSettingUsecase, v10, loggatewayLogger)
	skillTriggerGoldenRunner := provideSkillTriggerGoldenRunner(v44, v10, loggatewayLogger)
	...
	skillEvolutionSuggestionService := service.NewSkillEvolutionSuggestionService(skillIntelligenceUsecase, skillCuratorService, sandboxRunner, v13, agentUsecase, loggatewayLogger)
	...
	memoryFactSessionGrants := provideMemoryFactSessionGrants()
	memoryFactPendingDecider := provideMemoryFactPendingDecider(dataData, memoryFactSessionGrants, loggatewayLogger)
	twinOpenAPICompatService := service.NewTwinOpenAPICompatService(agentUsecase, graphUsecase, memoryAdminUsecase, v37, v20, llmProviderModelUsecase, memoryFactPendingDecider, loggatewayLogger)
	...
	decisionRecordService := service.NewDecisionRecordService(queryUsecase, v33, loggatewayLogger)
	...
	usecase := provideDiagnosticsUsecase(llmProviderModelUsecase, mcpServerUsecase, agentUsecase, dataData, v3, indexer, configgraphRepo, loggatewayLogger)
	diagnosticsService := provideDiagnosticsService(usecase, loggatewayLogger)
	...
	voiceWSServer := provideVoiceWSServer(sessionAuthorizer, wsTurnExecutor, runCanceller, speechRegistry, speechConfigReader, v2Bus, infra, loggatewayLogger, chatService, v23, delegationRegistry, stepV2Repo, multiProviderEmbedder)
	...
	eventBusSideConsumers := provideEventBusSideConsumers(infra, v2Bus, v29, webhookDispatcher, v33, v47, v20, turnMemoryWorker, v18, v19, v37, sessionLogWriter, flowLogWriter)
	...
	trpcBuilderDeps := provideTRPCBuilderDeps(llmProviderModelUsecase, v29, agentUsecase, agentRepository, systemSettingRepo, v13, persistenceSet, repository, factory, retriever, v35, manager, organizationUsecase, toolResultGate, router, subagentService, v22, bridge, runtime, lifecycle, loggatewayLogger)
	...
	factWriteAdjudicator := provideFactWriteAdjudicator(agentUsecase, v33, llmProviderModelUsecase, loggatewayLogger)
	memoryFactPendingNotifier := provideMemoryFactPendingNotifier(loggatewayLogger)
	factWritePipeline := provideFactWritePipeline(dataData, memoryUsecase, factWriteAdjudicator, memoryFactPendingNotifier, memoryFactSessionGrants, loggatewayLogger)
	agentCaseLLMExtractor := service.NewAgentCaseLLMExtractor(memoryLLMExtractor)
	writeBackArbiter := provideKnowledgeWriteBackArbiter(v35, dynamicLLMCaller, systemSettingUsecase, llmProviderModelUsecase, loggatewayLogger)
	mainKnowledgeDistillWiring := provideKnowledgeDistillWiring(v35, memoryAdminUsecase, dataData)
	autoMemoryWorker, err := provideAutoMemoryWorker(runtime, v33, agentUsecase, memoryConsolidationWriter, l4GraphWriter, memoryFactIndexSyncer, episodeIndexSyncer, memoryLLMExtractor, memoryJobQueue, memoryJobDeadLetterRepo, memoryWorkerStats, monitorBus, factWritePipeline, agentCaseLLMExtractor, memoryAgentCaseRepo, memoryAgentCaseRepo, knowledgeService, v35, dataData, loggatewayLogger, writeBackArbiter, mainKnowledgeDistillWiring)
	if err != nil {
		cleanup()
		return wireOut{}, nil, err
	}
	...
	deps2 := provideA2AGatewayHealthRunnerDeps(v22)
	...
	toolAuditCleanup := provideToolAuditCleanup(v29, loggatewayLogger)
	flowLogCleanup := provideFlowLogCleanup(v47, loggatewayLogger)
	monitorEventsCleanup := provideMonitorEventsCleanup(v5, loggatewayLogger)
	autoHealTTLCleanup := provideAutoHealTTLCleanup(v49, loggatewayLogger, flowLogWriter)
	monitorTraceBackfillWorker := provideMonitorTraceBackfillWorker(v6, v8, v17, loggatewayLogger)
	...
	knowledgeRelationExtractWorker := provideKnowledgeRelationExtractWorker(dataData, relationExtractor, v35, loggatewayLogger)
	knowledgeIndexRepairWorker := provideKnowledgeIndexRepairWorker(knowledgeService, loggatewayLogger)
	knowledgeCurateWorker := provideKnowledgeCurateWorker(v35, loggatewayLogger)
	...
	siAnalystStage := provideSIAnalystStage(selfImprovement, dynamicLLMCaller, systemSettingUsecase, repoSandboxRunner, rootCauseEngine, loggatewayLogger)
	...
	predictiveHealUsecase := providePredictiveHealUsecase(v20, failurePatternReadWriter, v49, llmProviderModelUsecase, mcpServerUsecase, loggatewayLogger)
	predictiveHealJob := providePredictiveHealJob(predictiveHealUsecase, loggatewayLogger)
	patternMiningUsecase := providePatternMiningUsecase(v49, failurePatternReadWriter, failurePatternReadWriter, loggatewayLogger)
	patternMiningJob := providePatternMiningJob(patternMiningUsecase, loggatewayLogger)
	memoryEnhancedExtractorConfig := provideMemoryEnhancedExtractorConfig(agentUsecase, v33, llmProviderModelUsecase, loggatewayLogger)
	...
	mainWireOut := provideWireOut(app, dataData, cronrunnerRunner, watchRunner, autoMemoryWorker, healthRunner, runner2, learningLoopScanner, providerHealthScanner, channelHealthScanner, jobsChannelDeliveryWorker, sessionRunDurableWorker, recoveryWorker, backgroundJobWorker, channelRuntime, plugintrpcRuntime, toolAuditCleanup, flowLogCleanup, monitorEventsCleanup, autoHealTTLCleanup, v51, monitorTraceBackfillWorker, memoryL2DecayWorker, memoryL1ArchiveWorker, channelTurnJobSweeper, memoryL3DecayWorker, memoryL4DecayWorker, memoryEbbinghausDecayWorker, memoryCanaryWorker, memoryCitationBackfillWorker, knowledgeCitationBackfillWorker, knowledgeRelationExtractWorker, knowledgeIndexRepairWorker, knowledgeCurateWorker, memorySleepTimeWorker, memoryEpisodeBackfillWorker, memoryDataMigrationWorker, memoryFactIndexReconciler, memoryDeadLetterReplayer, modelRegistrySyncAgent, selfCheckScheduler, v50, infra, selfCheckCleanup, selfCheckJob, cronRepo, skillIntelligenceUsecase, skillIntelligenceWorker, curatorWorker, evolutionOrchestratorWorker, selfImproveObserveWorker, selfImproveDriveWorker, selfImproveWatchdogWorker, selfImproveOutcomeWorker, failurePatternSyncJob, predictiveHealUsecase, predictiveHealJob, patternMiningUsecase, patternMiningJob, pathBExtractor, wsv2Subscriber, indexer)
...
func provideCodeExecutorFactory(lg loggateway.Logger, sandboxMgr *sandbox.Manager, sandboxLeases *sandbox.SessionLeases) *codeexecutor.Factory {
...
func provideChatServiceDeps(
	...
	skillIntel *biz.SkillIntelligenceUsecase, evolution2 *biz.EvolutionUsecase,
...
func provideMemoryService(persist runtime.PersistenceSet, cascade *biz.L4CascadeUsecase, sysUC *biz.SystemSettingUsecase, deadLetterRepo biz.MemoryDeadLetterAdminRepo, queue trpcmem.AutoMemoryQueue, queueStats *trpcmem.MemoryJobQueue, workerStats *biz.MemoryWorkerStats, d *data.Data, agentUC *biz.AgentUsecase, lg loggateway.Logger) *service.MemoryService {
...
func provideAgentFactory(
	...
	factory := agent.NewAgentFactoryImpl(llm, agentWriter, agentReader, templateRepo, eventBus, embedder, lg)
	if impl, ok := factory.(*agent.AgentFactoryImpl); ok {
