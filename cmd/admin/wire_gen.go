...
func wireApp(confServer *conf.Server, confData *conf.Data, runtime *conf.Runtime, debugRecorder *conf.DebugRecorder, logger log.Logger, loggatewayLogger loggateway.Logger, pipeline logpipeline.Pipeline, arg []*conf.LoggingSink) (wireOut, func(), error) {
	...
	v10 := skill.NewUsecase(skillRepo, multiProviderEmbedder)
	v11 := provideFilesystemHealthReader(v10)
	v12 := provideMonitorUsecase(v4, v5, v6, v7, v8, v9, v11, loggatewayLogger)
	v13 := data.NewA2ARepoFromData(dataData, loggatewayLogger)
	agentLookup := biz.ProvideA2AAgentLookup(agentRepository)
	v14 := a2a.NewUsecase(v13, v13, v13, v13, agentLookup)
	...
	v15 := artifact.NewUsecase(artifactRepo, loggatewayLogger)
	artifactService := provideArtifactRuntimeService(v15)
	...
	persistenceSet := providePersistenceSet(dataData, agentMCPTooling, sessionService, artifactService, v15, memoryService, memoryJobQueue, memoryPolicyEngine, memoryL2Recaller, memoryL3Recaller, memoryCompositeRecaller, memoryAdminUsecase, loggatewayLogger, memoryJobDeadLetterRepo)
	...
	agentService := service.NewAgentService(agentUsecase, evolutionUsecase, v12, v14, promptFileAIEditor, agentTemplateUsecase, loggatewayLogger)
	...
	v16 := hook.NewUsecase(hookRepo, loggatewayLogger)
	deliveryRepo := data.NewHookDeliveryRepo(dataData)
	v17 := hook.NewDeliveryUsecase(deliveryRepo)
	v18 := data.NewPluginRepo(dataData)
	v19 := data.NewPluginRunRepo(dataData)
	statsRecorder := providePluginStatsRecorder(v18, v19, agentRepository, runtime, loggatewayLogger)
	v20 := data.NewPluginCostGuardUsageRepo(dataData)
	...
	v21 := provideToolUsecaseWithDeps(v2, settingRepo, toolTester, toolWebResearchReadinessChecker, toolGrantRepo, loggatewayLogger)
	plugintrpcRuntime := providePluginRuntime(statsRecorder, v20, v21, deliveryRepo, monitorBus, loggatewayLogger)
	v22 := hook.NewResolver(v16, loggatewayLogger)
	manager := providePluginManager(plugintrpcRuntime, v22, agentRepository, loggatewayLogger)
	hookService := service.NewHookService(v16, v17, manager, loggatewayLogger)
	cronRepo := data.NewCronRepo(dataData)
	v23 := data.NewSessionMetricsRepo(dataData)
	v24 := data.NewSessionMetricsReader(dataData)
	sessionMetricsCache := data.NewSessionMetricsCache(v24, loggatewayLogger)
	sessionRepo := data.NewSessionRepo(dataData, v23, sessionMetricsCache)
	...
	v25 := session.NewSessionUsecase(sessionRepo, sessionAgentLookup, teamLookup, sessionTitleGenerator, sessionParticipantRepository, sessionStatusPublisher, sessionMetricsUsecase, sessionRuntimeWriter, activityLister, loggatewayLogger)
	...
	repository := service.NewSkillDBRepository(v10, loggatewayLogger)
	factory := provideCodeExecutorFactory(loggatewayLogger)
	v26 := data.NewKnowledgeRepoFromData(dataData)
	retriever := service.NewKnowledgeRetriever(multiProviderEmbedder, v26, loggatewayLogger)
	v27 := knowledge.NewUsecaseFromRepo(v26)
	...
	graphNodeResolverSet := provideGraphBuildDeps(llmProviderModelUsecase, v21, agentUsecase, agentRepository, systemSettingRepo, v10, persistenceSet, repository, factory, retriever, v27, manager, organizationUsecase, toolResultGate, router, subagentService, v14, nodeCircuitBreakerRegistry, loggatewayLogger)
	...
	v28 := data.NewSessionRuntimeReader(dataData)
	compiledTeamRepo := data.NewCompiledTeamRepo(dataData, v28)
	...
	v29 := provideUsageUsecase(v3, v12, teamUsecase, v25, v2Bus, loggatewayLogger)
	...
	v30 := data.NewKnowledgeSparseSearcherFromData(dataData)
	hybridRetriever := service.NewKnowledgeHybridRetriever(retriever, v30, loggatewayLogger)
	...
	federatedRetriever := service.NewKnowledgeFederatedRetriever(adaptiveRouter, retriever, v27, loggatewayLogger)
	...
	v31 := provideM71MessageReader(activityLister)
	globalMessageSearcher := data.NewGlobalMessageSearchRepo(dataData)
	sessionSearchUsecase := provideSessionSearchUsecase(agentRepository, sessionRepo, v31, globalMessageSearcher, accessAuditor, loggatewayLogger)
	runtimeTooling := provideRuntimeTooling(plugintrpcRuntime, manager, repository, retriever, adaptiveRouter, federatedRetriever, retrievalEvaluator, v27, factory, kanbanToolBridge, recorderFactory, organizationUsecase, toolResultGate, router, subagentService, parallelToolExecutor, resourceAccessUsecase, deptMailboxUsecase, sessionSearchUsecase)
	...
	turnDeps := provideTeamTurnDeps(v25, agentRepository, agentUsecase, v2, v21, llmProviderModelUsecase, v10, systemSettingRepo, providerReader, persistenceSet, sessionCompressor, v2Bus, monitorBus, sequencer, loggatewayLogger)
	projectorFactory := provideV2ProjectorFactory(sequencer, loggatewayLogger)
	runnerConfig := provideRunnerConfig(plugintrpcRuntime, manager, retriever, adaptiveRouter, federatedRetriever, retrievalEvaluator, v27, graphUsecase, graphBuilderFactory, taskUsecase, runRegistry, v21, agentRepository, organizationUsecase, toolResultGate, router, subagentService, kanbanToolBridge, v14, v25, projectorFactory, loggatewayLogger)
	runner := team.NewRunner(teamRepo, teamRepo, teamRepo, teamUsecase, teamRepo, teamRepo, v29, turnDeps, repository, factory, loggatewayLogger, runnerConfig)
	...
	spiritTeamUsecase := provideSpiritTeamUsecase(teamUsecase, v25, agentUsecase, spiritTransactor, orchestrationCache, evolutionSuggestionRepo, verificationGateExecutor, deptLeadManager, stepV2Repo, spiritTeamRunStatsReader, sessionRuntime, loggatewayLogger)
	...
	skillRegistrationPort := provideSkillRegistrationPort(v10)
	...
	experienceAnalyticsUsecase := biz.NewExperienceAnalyticsUsecase(evolutionMetricsRepo, skillRepo, teamRepo, teamRepo, v3, memoryAdminUsecase, sessionRepo, v2, loggatewayLogger)
	turnLifecycleUsecase := provideTurnLifecycleUsecase(v25, loggatewayLogger)
	...
	chatOrchestratorDeps := provideChatServiceDeps(runRegistry, pendingMessageQueue, v29, v25, agentRepository, agentUsecase, v2, v21, llmProviderModelUsecase, v10, systemSettingRepo, providerReader, persistenceSet, sessionRuntime, sessionCompressor, v2Bus, monitorBus, runtimeTooling, teamOrchestrationDeps, channelTurnJobDeps, channelNotifierDeps, v14, v15, mcpServerUsecase, v12, spiritTeamAssembler, spiritSynthesisService, orchestrationCache, teamStarter, graphService, taskOrchestratorPort, skillEvolutionUsecase, evolutionUsecase, skillInvocationStatsReader, router, subagentService, experienceAnalyticsUsecase, turnLifecycleUsecase, stepV2Repo, stepV2Repo, taskV2Repo, runHeartbeatEmitter, deadLetterQueue, profileResolver, projectorFactory, memberSessionV2Repo, memoryConsolidationWriter, loggatewayLogger)
	...
	deps := provideCronRunnerDeps(cronRepo, v25, teamRepo, agentRepository, v2Bus, monitorBus, chatService, modelRegistrySyncAgent)
	cronrunnerRunner := provideCronRunner(deps, dataData, loggatewayLogger)
	v32 := cron.NewUsecase(cronRepo, cronrunnerRunner)
	cronService := service.NewCronService(v32)
	scopeAgentLookup := biz.NewScopeAgentLookup(agentRepository)
	v33 := plugin.NewUsecase(v18, v19, scopeAgentLookup)
	pluginService := service.NewPluginServiceWithBootstrap(v33, plugintrpcRuntime, loggatewayLogger)
	mcpServerService := service.NewMCPServerService(mcpServerUsecase, v12)
	...
	v34 := service.ProvideSkillResolveRootFn(systemSettingRepo)
	v35 := storage.NewSkillFilesystem(v34)
	...
	skillService := service.NewSkillService(v10, agentUsecase, skillHealthUsecase, v35, engine, loggatewayLogger)
	toolService := service.NewToolService(v21, v12)
	...
	serviceSessionService := service.NewSessionService(v25, v12, sessionRunUsecase, sessionCompressor, sessionCompressor, v24, sessionV2Service, loggatewayLogger)
	cronTriggerGateway := service.NewCronTriggerGatewayAdapter(cronService)
	turnAdmissionUsecase := provideChannelIngressAdmission(v29, agentRepository, channelUsecase)
	teamCompiler := provideTeamCompiler(channelUsecase, loggatewayLogger)
	channelIngress := provideChannelIngress(channelUsecase, channelTurnJobUsecase, v25, chatService, graphService, cronTriggerGateway, v2Bus, monitorBus, turnAdmissionUsecase, teamCompiler, loggatewayLogger)
	...
	usageService := service.NewUsageService(v29)
	processLogEnabledProvider := provideProcessLogEnabled(confServer)
	flowlogRepo := data.NewFlowLogRepo(dataData)
	v36 := flowlog.NewUsecase(flowlogRepo)
	flowLogService := service.NewFlowLogService(v36)
	...
	v37 := provideDiagBundleGenerator(v5, v6, rootCauseEngine)
	v38 := provideSelfHealUsecase(v37, loggatewayLogger)
	v39 := data.NewHealRecordRepo(dataData)
	v40, err := provideSelfHealObserver(runtime, v39, rootCauseEngine, v9, loggatewayLogger)
	if err != nil {
		cleanup()
		return wireOut{}, nil, err
	}
	dbPinger := provideDBPinger(dataData)
	traceProjector := provideTraceProjector(v6, infra, loggatewayLogger)
	alertEvalWorker := provideMonitorAlertEvalWorker(v12)
	...
	v41 := monitor.ProvideSelfCheckers(dbPinger, traceProjector, alertEvalWorker, eventBusHealthChecker, wsConnectionCounter, flowFileAppender)
	eventBusResubscriber := provideEventBusResubscriber()
	v42 := monitor.ProvideSelfCheckRepairers(flowFileAppender, traceProjector, alertEvalWorker, eventBusResubscriber)
	selfCheckReportRepo := data.NewSelfCheckReportRepo(dataData)
	alertMetricRegistry := monitor.NewAlertMetricRegistry()
	selfCheckScheduler := provideSelfCheckScheduler(v41, v42, selfCheckReportRepo, alertMetricRegistry, loggatewayLogger)
	monitorService := service.NewMonitorService(v12, processLogEnabledProvider, flowLogService, codeExecutorService, v37, v38, v40, selfCheckScheduler, selfCheckReportRepo, loggatewayLogger)
	...
	endpointRegistry := provideA2AEndpointRegistry(a2AEndpointBuilder, v14, publicBaseURLStore, loggatewayLogger)
	...
	teamService := service.NewTeamService(teamUsecase, graphUsecase, agentUsecase, v25, runner, runRegistryPort, v2Bus, loggatewayLogger, spiritSynthesisService, teamStageV2Repo, stepV2Repo, stepV2Repo, teamRunV2Repo, memberSessionV2Repo, sequencer)
	signer := provideArtifactSigner(loggatewayLogger)
	sessionWorkspaceLookup := service.ProvideSessionWorkspaceLookup(v25)
	sessionWorkspaceSearcher := service.ProvideSessionWorkspaceSearcher(v25)
	serviceArtifactService := service.NewArtifactService(v15, signer, sessionWorkspaceLookup, sessionWorkspaceSearcher)
	...
	knowledgeService := service.NewKnowledgeService(v27, multiProviderEmbedder, knowledgeSearchDeps, markdownOrganizer, extractorRegistry, assetStore, v2Bus, systemSettingRepo, loggatewayLogger)
	evaluationRepo := data.NewEvalRepoFromData(dataData)
	v43 := evaluation.NewUsecase(evaluationRepo, loggatewayLogger)
	evaluationRunner := service.ProvideEvaluationRunner(chatService, chatService, v43, llmProviderModelUsecase, systemSettingRepo, loggatewayLogger)
	...
	a2AService := provideA2AService(v14, chatService, agentRepository, endpointRegistry, publicBaseURLStore, limiter, loggatewayLogger)
	ecosystemRepo := data.NewEcosystemRepo(dataData)
	v44 := ecosystem.NewUsecase(ecosystemRepo, loggatewayLogger)
	ecosystemService := service.NewEcosystemService(v44)
	...
	eventBusSideConsumers := provideEventBusSideConsumers(infra, v2Bus, v21, webhookDispatcher, v25, v36, v12, turnMemoryWorker, traceProjector, flowFileAppender, v29, sessionLogWriter)
	...
	sessionStatusGuard := service.NewSessionStatusGuard(v25, teamUsecase, taskOrchestratorPort, v2Bus, v2RecoveryRepo, chatService, loggatewayLogger)
	...
	memoryLLMExtractorConfig := provideMemoryLLMExtractorConfig(agentUsecase, v25, llmProviderModelUsecase, loggatewayLogger)
	memoryLLMExtractor := service.NewMemoryLLMExtractor(memoryLLMExtractorConfig)
	autoMemoryWorker, err := provideAutoMemoryWorker(runtime, v25, agentUsecase, memoryConsolidationWriter, l4GraphWriter, memoryFactIndexSyncer, episodeIndexSyncer, memoryLLMExtractor, memoryJobQueue, memoryJobDeadLetterRepo, memoryWorkerStats, loggatewayLogger)
	if err != nil {
		cleanup()
		return wireOut{}, nil, err
	}
	...
	deps2 := provideA2AGatewayHealthRunnerDeps(v14)
	...
	toolAuditCleanup := provideToolAuditCleanup(v21, loggatewayLogger)
	flowLogCleanup := provideFlowLogCleanup(v36, loggatewayLogger)
	monitorAlertCooldownCleanup := provideMonitorAlertCooldownCleanup(v12, logger)
	autoHealTTLCleanup := provideAutoHealTTLCleanup(v39, loggatewayLogger, logger)
	monitorTraceBackfillWorker := provideMonitorTraceBackfillWorker(v6, v8, loggatewayLogger)
	...
	predictiveHealUsecase := providePredictiveHealUsecase(v12, failurePatternReadWriter, v39, loggatewayLogger)
	predictiveHealJob := providePredictiveHealJob(predictiveHealUsecase, loggatewayLogger)
	patternMiningUsecase := providePatternMiningUsecase(v39, failurePatternReadWriter, failurePatternReadWriter, loggatewayLogger)
	patternMiningJob := providePatternMiningJob(patternMiningUsecase, loggatewayLogger)
	memoryEnhancedExtractorConfig := provideMemoryEnhancedExtractorConfig(agentUsecase, v25, llmProviderModelUsecase, loggatewayLogger)
