...
func wireApp(confServer *conf.Server, confData *conf.Data, runtime *conf.Runtime, selfImprovement *conf.SelfImprovement, sandbox *conf.Sandbox, debugRecorder *conf.DebugRecorder, logger log.Logger, loggatewayLogger loggateway.Logger, pipeline logpipeline.Pipeline, arg []*conf.LoggingSink) (wireOut, func(), error) {
	...
	runtimeTooling := provideRuntimeTooling(plugintrpcRuntime, manager, repository, skillHealthMetricsAdapter, retriever, adaptiveRouter, federatedRetriever, retrievalEvaluator, knowledgeUsecase, factory, kanbanToolBridge, recorderFactory, organizationUsecase, toolResultGate, router, subagentService, parallelToolExecutor, resourceAccessUsecase, deptMailboxUsecase, sessionSearchUsecase, bridge, computerUseUsecase, agentBridgeService, sessionLeases, runtime)
	...
	spiritTeamUsecase := provideSpiritTeamUsecase(teamUsecase, sessionUsecase, agentUsecase, spiritTransactor, orchestrationCache, evolutionUsecase, verificationGateExecutor, deptLeadManager, deptMailboxUsecase, stepV2Repo, spiritTeamRunStatsReader, sessionRuntime, systemSettingRepo, loggatewayLogger)
	...
	sessionForkStore := data.NewSessionForkRepo(dataData)
	sessionForkUsecase := biz.ProvideSessionForkUsecase(sessionRepo, sessionForkStore, loggatewayLogger)
	...
	serviceSessionService := service.NewSessionService(sessionUsecase, sessionForkUsecase, monitorUsecase, sessionRunUsecase, sessionCompressor, sessionCompressor, sessionMetricsReader, sessionV2Service, loggatewayLogger)
	...
	teamService := service.NewTeamService(teamUsecase, graphUsecase, agentUsecase, sessionUsecase, runner, runRegistryPort, v2Bus, loggatewayLogger, spiritSynthesisService, teamStageV2Repo, stepV2Repo, stepV2Repo, teamRunV2Repo, memberSessionV2Repo, sequencer, monitorUsecase, usageUsecase, queryUsecase)
	...
	skillEvolutionSuggestionService := service.NewSkillEvolutionSuggestionService(skillIntelligenceUsecase, skillCuratorService, sandboxRunner, skillUsecase, agentUsecase, loggatewayLogger)
	...
	twinOpenAPICompatService := service.NewTwinOpenAPICompatService(agentUsecase, graphUsecase, memoryAdminUsecase, usageUsecase, monitorUsecase, llmProviderModelUsecase, memoryFactPendingDecider, loggatewayLogger)
	...
	mainKnowledgeDistillWiring := provideKnowledgeDistillWiring(knowledgeUsecase, memoryAdminUsecase, dataData)
