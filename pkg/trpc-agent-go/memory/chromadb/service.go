...
// ProactiveRecall retrieves associated memories based on the conversation context.
// The chromadb implementation delegates to SearchMemories for each mentioned entity
// and the current topic, then merges and deduplicates results.
func (svc *Service) ProactiveRecall(ctx context.Context, userKey memory.UserKey,
	...
	for _, q := range queries {
		entries, err := svc.SearchMemories(ctx, userKey, q)
		if err != nil {
			return nil, err
		}
