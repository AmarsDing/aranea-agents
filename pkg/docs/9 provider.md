...
本文档描述 **Provider 列表页**、**添加/编辑 Provider** 弹窗及 **Embedding** 子配置；与 **`8 agent-title.md` §8.1**（供应商–模型级联）、**`2 agents-create.md`**（Provider/模型选择）、**`前端.md`** `agents.provider` 对齐。
...
资源对应表 **`llm_provider_models`**；路径可保留 **面向产品的** `/llm-providers` 前缀，由后端映射到行级 `id` 或 `(provider_code, model_api_id)`。
...
- [ ] **Embedding** 开关与记忆/嵌入链路配置一致（若已接）。  

---

*文档版本：与 `8 agent-title.md` §8.1、`2 agents-create.md` 对齐。*
