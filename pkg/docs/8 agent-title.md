...
| **模型** | `QSelect` | **级联**：仅在已选供应商后启用；选项为 **同 `provider_code` 下各行的 `model_api_id` / `model_display_name`**。切换供应商时 **清空模型** 并 `GET ...?provider_code=`（见 **`9 provider.md` §5 / §7**） |
...
| GET | `/llm-providers` 或 `/llm-provider-models` | 供应商/模型数据；底层 **`llm_provider_models`**（可按 `provider_code` 聚合，见 **`9 provider.md` §7**） |
