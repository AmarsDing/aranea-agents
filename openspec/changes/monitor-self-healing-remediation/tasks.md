...
- [ ] 1.4 编写 `internal/data/monitor_circuit_test.go` 单元测试。DoD: `go test ./internal/data/... -run TestCircuitBreaker -count=1` 绿色
...
- [ ] 2.2 编写单元测试。DoD: `go test ./internal/cronrunner/... -run TestAutoHealTTLCleanup -count=1` 绿色

## 3. 补齐单元测试

- [ ] 3.1 编写 Phase 1.1 单元测试（`WithAutoHealConfig` RunOption）。DoD: `go test ./internal/biz/... -run TestAutoHealConfig -count=1` 绿色
- [ ] 3.2 编写 Phase 1.3 集成测试（auto_healed FlowLog 发射）。DoD: `go test ./internal/data/... -run TestAutoHealFlowLog -count=1` 绿色
- [ ] 3.3 编写 Phase 2.3 SelfHealObserver 单元测试。DoD: `go test ./internal/biz/... -run TestSelfHealObserver -count=1` 绿色
