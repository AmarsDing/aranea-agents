...
- [x] T1.1: 修复 P0 Bug — DAG 团队状态矛盾
  - Spec: spirit-team/REQ-ST-02
  - 修复 assembleDAGTeams 中 AutoStart=false 的无依赖节点初始状态为 active 但永不启动的问题。改为 pending 状态。
...
- [x] T1.2: Team 状态机归一
  ...
  - 删除 active/assembled 状态，新增 pending/interrupted 状态，定义合法状态转换。迁移脚本将现有 active→pending/running。
...
- [x] T1.4: 扩展 Intent Pass 输出
  - Spec: task-planner/REQ-TP-01
  - 在 Artifact 结构中增加 complexity_score, complexity_signals, suggested_agents, suggested_topology 字段。扩展 intentSystemCoding 和 intentSystemGeneral 的 system prompt。
  - Files: internal/agent/intent/pass.go
  - Acceptance: Intent Pass 输出包含新字段；新字段有合理默认值

- [x] T1.5: 实现 TaskPlanner 端口和基础实现
  - Spec: task-planner/REQ-TP-01, REQ-TP-03, REQ-TP-04, REQ-TP-06
  - 定义 TaskPlannerPort 接口，实现六维复杂度评估（语义+结构+工具+上下文+历史，领域跨度 Phase 3），策略输出，TaskPlan 持久化。AI 任务拆分（REQ-TP-02）在 T1.6 单独实现。
...
- [x] T1.6: 实现 AI 任务拆分
  ...
  - 实现 complex 级别任务的 LLM 拆分。结构化 prompt 约束输出 SubTask[] + required_capabilities + depends_on。生成 TaskDAG。
  - Files: internal/agent/task_planner_impl.go
  - Acceptance: complex 任务被拆分为 SubTask[]；每个 SubTask 含 required_capabilities；TaskDAG 无环

- [x] T1.7: 实现 moderate 路径（Agent-as-Tool）
  - Spec: spirit-team/REQ-ST-03
  - 使用 agenttool.NewTool 将匹配的 Agent 包装为工具，Spirit 调用该工具完成任务。删除 list_butlers / query_butler_status。
...
- [x] T1.8: StepID 注册表 + spirit_trace_id
  - Spec: spirit-observability/REQ-SO-01, REQ-SO-02
  - 创建 spirit_step_ids.go 定义所有 StepID 常量。在 ChatOrchestrator 的 turn 入口生成 spirit_trace_id 并贯穿三阶段。
...
- [x] T1.9: 启动时自动恢复扩展
  ...
  - 扩展 RecoverOrphanedRunningSessions 同时恢复 Team 状态（running→interrupted）和 TeamRun 状态（running→failed）。
