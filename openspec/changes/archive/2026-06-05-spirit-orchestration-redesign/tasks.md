...
### T1.7: 实现 moderate 路径（Agent-as-Tool） ⚠️
- **ID**: T1.7
...
- **Status**: [x] 已实现（偏差：list_butlers/query_butler_status 仍以 DEPRECATED 标记注册，见 DEV-08）

### T1.8: StepID 注册表 + spirit_trace_id ⚠️
- **ID**: T1.8
...
- **Test**: 日志输出验证测试
- **Status**: [x] 部分实现（偏差：spirit_trace_id 未在 ChatOrchestrator turn 入口生成，仅在后两阶段生成，见 DEV-11）
...
### T2.1: 实现 AgentAllocator 端口和基础实现 ⚠️
- **ID**: T2.1
...
- **Status**: [x] 已实现（偏差：Layer 2 使用 TF-IDF 占位，非 Embedding，见 DEV-05）

### T2.2: 实现 Agent 能力注册表和执行历史 ⚠️
- **ID**: T2.2
...
- **Test**: 能力构建测试；性能更新测试
- **Status**: [x] 已实现（偏差：AgentCapability.Capacity 字段存在但未使用，见 DEV-03）

### T2.3: 实现 TaskOrchestrator 端口和 DAG 编排 ⚠️
- **ID**: T2.3
...
- **Status**: [x] 已实现（偏差：DAG 编译后的 Definition JSON 未实际替换 Team 的 DefinitionJSON，仅用于日志，见 DEV-01）
...
- **Test**: 新旧工具集成测试
- **Status**: [x] 已实现（偏差：旧工具仍以 DEPRECATED 标记注册，双写过渡中，见 DEV-08；spirit profile 仍引用旧工具名，见 DEV-04）
...
- **Test**: 语义匹配测试
- **Status**: [ ] 未实现（当前使用 TF-IDF 占位，见 DEV-05）
