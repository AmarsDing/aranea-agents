...
- [x] 5.12 前端适配：处理 `MetricsUpdated` 事件，更新本地 session metrics 状态；修复 `reconcilePatchFromServer` 旧值覆盖问题
  - DoD: ✅ `metrics_updated` 事件已处理（fetchAndReconcileSession），`reconcilePatchFromServer` 对 context_used_ratio/context_used_tokens 改用 Math.max 防止旧值覆盖，context_status 从 reconciled ratio 派生，`pnpm build` 通过
- [x] 5.13 数据一致性验证脚本：对比 sessions 旧字段与 session_metrics/session_runtime 新表数据，输出差异报告
  - DoD: ✅ `cmd/session-consistency-check/main.go` 已存在，对比 17 个 metrics 字段 + 5 个 runtime 字段，含缺失/孤立行检测，`go build` 通过

## 6. VectorStore 策略模式

- [x] 6.1 创建 `internal/data/vector/store.go`：定义 `VectorStore` 接口（Upsert / Search / Delete）+ `VectorHit` struct
  - DoD: ✅ 接口定义完成，含 Upsert/Search/Delete + VectorHit struct
...
- [x] 6.3 创建 `internal/data/vector/pgvector.go`：实现 `PgVectorStore`，使用 pgvector 扩展
  - DoD: ✅ 含 build tag `pgvector`，无 pgvector 时走 stub
...
- [x] 6.5 修改 memory_facts 表：DDL migration 添加 `embedding_ref TEXT` 列，后续版本移除 `embedding_blob` / `embedding_norm` 列
  - DoD: ✅ `ddlVectorEmbeddingRef` migration 已注册（version 20260709）
- [x] 6.6 迁移 L3FactRepo / L2EpisodeRepo 的向量操作到 VectorStore
  - DoD: ✅ 新增 `store_vector_recall.go`（RecallL3FactsWithVectorStore / RecallL2EpisodesWithVectorStore），shim repos 在 VectorStore 可用时优先走 VectorStore，wire 重新生成，`go build ./...` 通过
...
- [x] 7.6 创建 Ent Schema `channel_runtime_lease.go`
  - DoD: ✅ 同上
- [x] 7.7 迁移 6 个 Raw SQL Repo 到 Ent API：`sessionRunRepo`、`sessionParticipantRepo`、`sessionRunCheckpointRepo`、`channelInboundReceiptRepo`、`channelTurnJobRepo`、`channelRuntimeLeaseRepo` — 优先使用 Ent CRUD API，仅保留特殊 UPSERT 逻辑为 Raw SQL
  - DoD: ✅ sessionParticipantRepo/sessionRunCheckpointRepo 完全迁移；sessionRunRepo(9/12 Ent)、channelTurnJob(2/6 Ent)、channelRuntimeLease(2/3 Ent) 已迁移可迁移部分；channelInboundReceipt 保留 ON CONFLICT DO NOTHING 的 Raw SQL（符合设计意图）
- [x] 7.8 更新 Wire 绑定和 `data.ProviderSet`
  - DoD: ✅ `make wire && go build ./...` 通过

## 8. Schema 真相源统一

- [x] 8.1 增强 `ddl_migration_registry.go`：支持 `SQL` 字段（嵌入 SQL 文件路径），在执行时读取并执行 SQL 文件内容
  - DoD: ✅ 新增 `SQL` 字段 + `executeSQLFile` 函数 + `migrationSQLFS` embed.FS，SQL 文件已复制到 `internal/data/sql/migrations/`
- [x] 8.2 从 `memory_chain.sql` 中删除与 Ent Schema 重叠的 24 张表定义
  - DoD: ✅ `memory_chain.sql` 仅包含 34 张 Memory 专属表（L0-L4 + cascade + action_log + agent evolution + monitor + audit 等）
- [~] 8.3 将 12 个 `*_patch.go` 中的 ALTER TABLE 逻辑迁移到 DDL migration SQL 文件
  ...
  - **延后**：12 个 patch 文件已通过 DDL migration registry 注册并正常工作，迁移到 SQL 文件是纯重构且风险高（需要逐一验证每个 ALTER TABLE 语句），当前系统已正常工作
- [~] 8.4 创建迁移测试 helper：`internal/data/testhelper/migration.go`，提供 `SetupTestDB(t)` 函数，自动执行所有 DDL migration
  ...
  - **延后**：当前测试已通过 `EnsureSchema` 初始化，创建新 helper 是锦上添花，非关键路径
