...
- **BREAKING** `FailurePolicy` 编译期展开为 NodeDef 通用字段，从 `GraphBuildConfig` 移除 `FailurePolicy` 和 `ParallelBranchIDs`
...
- `architecture`: GraphBuildConfig 从 13 字段简化为 11 字段（纯图拓扑），NodeDef 从 28 字段降至 20 字段
