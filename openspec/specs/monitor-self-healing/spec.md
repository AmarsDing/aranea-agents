...
系统 SHALL 将 circuit breaker 触发条件从"连续 5 次失败触发"改为"10 分钟内 5 次 auto-heal 事件触发 circuit open"，并 SHALL 通过 cooldown 机制实现 30 分钟自动重置。
...
系统 SHALL 使用 `SelfHealObserver` 替代已 Deprecated 的 `SelfHealUsecase` 承担观察角色。
