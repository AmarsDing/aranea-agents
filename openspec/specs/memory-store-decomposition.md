...
**Status**: ✅ Implemented. `sessionmemory.Store` has been fully deleted. All 6 Repos hold `*Data` and implement biz interfaces directly via Raw SQL through `ReadWriteDB`.
...
**Status**: ✅ Implemented. Adapters are in `internal/data/memory_admin_adapter.go`.
...
**Status**: ✅ Implemented. `sessionmemory.Store` has been deleted. All biz interfaces are satisfied by independent Repo structs.
...
**Status**: ⏸ Deferred. Shim repos must implement biz interfaces (method signatures use biz types). True DTO decoupling happens when repos no longer need to satisfy biz interfaces directly.
...
**Status**: ✅ Completed (shim phase over). All repos now implement biz interfaces directly via Raw SQL. The `sessionmemory.Store` has been deleted.
