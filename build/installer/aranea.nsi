...
; Usage: makensis /DVERSION=0.1.32 /DSTAGING_DIR=build\staging /DOUT_DIR=build\release aranea.nsi
...
; Default-checked: after install the launcher opens the status console, runs
; environment checks and (first run) the interactive config wizard.
!define MUI_FINISHPAGE_RUN "$INSTDIR\AraneaLauncher.exe"
!define MUI_FINISHPAGE_RUN_TEXT "Start Aranea-Agents (environment check + first-run setup)"
...
Section "MainSection" SecMain
  ...
  CreateShortcut "$SMPROGRAMS\Aranea-Agents\Configure (Setup Wizard).lnk" "$INSTDIR\AraneaLauncher.exe" "-setup" "$INSTDIR\frontend\icon.ico"
...
Section "Uninstall"
  ; Remove boot auto-start registration (Windows service / logon scheduled task)
  ; before killing processes, so no dangling SCM entries or tasks survive.
  IfFileExists "$INSTDIR\AraneaLauncher.exe" 0 skip_autostart
    nsExec::ExecToLog '"$INSTDIR\AraneaLauncher.exe" -uninstall-autostart'
  skip_autostart:
  nsExec::ExecToLog 'taskkill /IM AraneaAgents.exe /F /T'
  nsExec::ExecToLog 'taskkill /IM aranea-server.exe /F /T'
  nsExec::ExecToLog 'taskkill /IM AraneaLauncher.exe /F /T'
