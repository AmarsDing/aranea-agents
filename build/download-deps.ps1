...
.EXAMPLE
    ...
    .\build\download-deps.ps1 -LocalDepsDir .\AraneaAgents-deploy

.EXAMPLE
    # 从官方 URL 下载（不含 pgvector，需手动添加）
    .\build\download-deps.ps1

.EXAMPLE
    # 从 GitHub Release 下载预打包依赖（含 pgvector）
    .\build\download-deps.ps1 -FromRelease -ReleaseTag deps-v1
#>
param(
...
if (-not (Test-Path $vectorDll)) {
    $bundledVector = Join-Path $RepoRoot "build\installer\bundled\pgvector-pg18"
